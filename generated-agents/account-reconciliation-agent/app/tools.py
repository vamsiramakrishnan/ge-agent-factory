"""Auto-generated fixture-backed tools for: account_reconciliation_agent"""
...
_CONTRACT_DOCUMENTS = [{"id":"account-reconciliation-agent-controls-playbook","title":"Account Reconciliation Agent Controls Playbook","type":"policy","path":"documents/account-reconciliation-agent-controls-playbook.md","source_system_id":"bigquery"}]
...
def list_documents() -> dict[str, Any]:
    ...
    return {"documents": [{"id":"account-reconciliation-agent-controls-playbook","title":"Account Reconciliation Agent Controls Playbook","type":"policy","path":"documents/account-reconciliation-agent-controls-playbook.md","wordCount":510}]}

def read_document(document_id: str) -> dict[str, Any]:
    """Read the full text of a document by its ID. Use for citing evidence."""
    docs = [{"id":"account-reconciliation-agent-controls-playbook","path":"documents/account-reconciliation-agent-controls-playbook.md","title":"Account Reconciliation Agent Controls Playbook"}]
...
def search_documents(query: str = "") -> dict[str, Any]:
    ...
    results = []
    for doc_meta in [{"id":"account-reconciliation-agent-controls-playbook","path":"documents/account-reconciliation-agent-controls-playbook.md","title":"Account Reconciliation Agent Controls Playbook"}]:
...
def lookup_account_reconciliation_agent_controls_playbook(section_anchor: str = "", document_id: str = "") -> dict[str, Any]:
...
def action_sap_s_4hana_fi_close(target_id: str = "", rationale: str = "") -> dict[str, Any]:
    ...
    audit_trail = "action_sap_s_4hana_fi_close(target_id={target_id}, rationale={rationale})".format(target_id=target_id, rationale=rationale)
    action_id = _deterministic_id("ACTION_I", "action_sap_s_4hana_fi_close", target_id, rationale)
    audit_record_id = _deterministic_id("AUDIT_RE", "action_sap_s_4hana_fi_close", target_id, rationale)
    result = {
...
_AGENT_ID = _os.environ.get("GE_AGENT_ID", "account_reconciliation_agent")
# Registry server id this agent registers as (override via env if needed).
_MCP_SERVER = _os.environ.get("GE_MCP_SERVER", "account-reconciliation-agent")
...
def _mcp_toolsets():
    ...
    toolsets = []
    for _name in [
        "mcpServers/account-reconciliation-agent",
    ]:
        try:
