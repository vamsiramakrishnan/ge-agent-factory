"""Auto-generated smoke tests for: ap_policy_compliance_monitor"""
import json, csv, os
...
class TestFixtures:
    def test_manifest(self):
        ...
        assert m["id"] == "ap_policy_compliance_monitor"
    ...
    def test_requisitions_exists(self):
        p = FIX / "tables/requisitions.json"
        assert p.exists()
        data = json.loads(p.read_text()) if p.suffix == ".json" else list(csv.DictReader(p.open(newline="")))
        assert len(data) > 0

    def test_purchase_orders_exists(self):
        p = FIX / "tables/purchase_orders.json"
        assert p.exists()
        data = json.loads(p.read_text()) if p.suffix == ".json" else list(csv.DictReader(p.open(newline="")))
        assert len(data) > 0

    def test_invoices_exists(self):
        p = FIX / "tables/invoices.json"
        assert p.exists()
        data = json.loads(p.read_text()) if p.suffix == ".json" else list(csv.DictReader(p.open(newline="")))
        assert len(data) > 0
...
class TestTools:
    ...
    def test_query_coupa_requisitions(self):
        from app.tools import query_coupa_requisitions
        r = query_coupa_requisitions(limit=5)
        assert r["total"] > 0
        assert r.get("source_system")

    def test_query_coupa_purchase_orders(self):
        from app.tools import query_coupa_purchase_orders
        r = query_coupa_purchase_orders(limit=5)
        assert r["total"] > 0
        assert r.get("source_system")

    def test_query_coupa_invoices(self):
        from app.tools import query_coupa_invoices
        r = query_coupa_invoices(limit=5)
        assert r["total"] > 0
        assert r.get("source_system")
...
class TestBehaviorContract:
    def test_contract_tools_importable(self):
        ...
        for fn in ["query_sap_s_4hana_fi_gl_entries","query_coupa_requisitions","query_bigquery_analytics_events","lookup_ap_policy_compliance_monitor_controls_playbook"]:
