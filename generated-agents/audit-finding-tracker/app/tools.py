"""Auto-generated fixture-backed tools for: audit_finding_tracker"""
...
_CONTRACT_DOCUMENTS = [{"id":"audit-finding-tracker-controls-playbook","title":"Audit Finding Tracker Controls Playbook","type":"policy","path":"documents/audit-finding-tracker-controls-playbook.md","source_system_id":"auditboard"}]
...
def list_systems() -> dict[str, Any]:
    ...
    return {"scenario": "audit_finding_tracker", "systems": [{"id":"auditboard","name":"AuditBoard","owns":["auditboard_records","auditboard_events","auditboard_audit_trail"],"protocol":"REST API","localBacking":["alloydb"],"toolNames":["query_auditboard_auditboard_records","query_auditboard_auditboard_events","query_auditboard_auditboard_audit_trail"],"evidence":["source_system_record","generated_audit_trail"]},{"id":"servicenow","name":"ServiceNow","owns":["tickets","change_requests","incidents"],"protocol":"REST API","localBacking":["alloydb"],"toolNames":["query_servicenow_tickets","query_servicenow_change_requests","query_servicenow_incidents"],"evidence":["source_system_record","generated_audit_trail"]},{"id":"finance_3","name":"FINANCE 3","owns":["finance_3_records","finance_3_events"],"protocol":"REST API","localBacking":["alloydb"],"toolNames":["query_finance_3_records"],"evidence":["source_system_record"]}], "tables": [{"name":"auditboard_records","sourceSystem":"AuditBoard","sourceSystemId":"auditboard"},{"name":"auditboard_events","sourceSystem":"AuditBoard","sourceSystemId":"auditboard"},{"name":"auditboard_audit_trail","sourceSystem":"AuditBoard","sourceSystemId":"auditboard"},{"name":"tickets","sourceSystem":"ServiceNow","sourceSystemId":"servicenow"},{"name":"change_requests","sourceSystem":"ServiceNow","sourceSystemId":"servicenow"},{"name":"incidents","sourceSystem":"ServiceNow","sourceSystemId":"servicenow"},{"name":"finance_3_records","sourceSystem":"FINANCE 3","sourceSystemId":"finance_3"},{"name":"finance_3_events","sourceSystem":"FINANCE 3","sourceSystemId":"finance_3"}]}
...
def query_servicenow_tickets(lookup_key: str = "", date_range: str = "", id: str = "", title: str = "", priority: str = "", status: str = "", assignee: str = "", created_at: str = "", limit: int = 20) -> dict[str, Any]:
    """Query ServiceNow tickets. Columns: id, title, priority, status, assignee, created_at, category, sla_met."""
    rows = _json("tables/tickets.json")
    if lookup_key:
        needle = lookup_key.lower()
        rows = [r for r in rows if any(needle in str(value).lower() for value in r.values())]
    if date_range:
    ...
    if priority: rows = [r for r in rows if str(r.get("priority","")).lower() == priority.lower()]
    if status: rows = [r for r in rows if str(r.get("status","")).lower() == status.lower()]
    if assignee: rows = [r for r in rows if str(r.get("assignee","")).lower() == assignee.lower()]
    if created_at: rows = [r for r in rows if str(r.get("created_at","")).lower() == created_at.lower()]
    return {"source_system": "ServiceNow", "source_system_id": "servicenow", "table": "tickets", "rows": rows[:max(1,min(limit,100))], "total": len(rows), "produces": ["tickets_records","tickets_summary"], "evidence": ["source_system_record"]}

def query_servicenow_change_requests(id: str = "", title: str = "", priority: str = "", status: str = "", assignee: str = "", created_at: str = "", limit: int = 20) -> dict[str, Any]:
    """Query ServiceNow change_requests. Columns: id, title, priority, status, assignee, created_at, category, sla_met."""
    rows = _json("tables/change_requests.json")
    if id: rows = [r for r in rows if str(r.get("id","")).lower() == id.lower()]
    if title: rows = [r for r in rows if str(r.get("title","")).lower() == title.lower()]
    if priority: rows = [r for r in rows if str(r.get("priority","")).lower() == priority.lower()]
    if status: rows = [r for r in rows if str(r.get("status","")).lower() == status.lower()]
    if assignee: rows = [r for r in rows if str(r.get("assignee","")).lower() == assignee.lower()]
    if created_at: rows = [r for r in rows if str(r.get("created_at","")).lower() == created_at.lower()]
    return {"source_system": "ServiceNow", "source_system_id": "servicenow", "table": "change_requests", "rows": rows[:max(1,min(limit,100))], "total": len(rows), "produces": ["change_requests_records","change_requests_summary"], "evidence": ["source_system_record"]}

def query_servicenow_incidents(id: str = "", title: str = "", priority: str = "", status: str = "", assignee: str = "", created_at: str = "", limit: int = 20) -> dict[str, Any]:
    """Query ServiceNow incidents. Columns: id, title, priority, status, assignee, created_at, category, sla_met."""
    rows = _json("tables/incidents.json")
    if id: rows = [r for r in rows if str(r.get("id","")).lower() == id.lower()]
    if title: rows = [r for r in rows if str(r.get("title","")).lower() == title.lower()]
    if priority: rows = [r for r in rows if str(r.get("priority","")).lower() == priority.lower()]
    if status: rows = [r for r in rows if str(r.get("status","")).lower() == status.lower()]
    if assignee: rows = [r for r in rows if str(r.get("assignee","")).lower() == assignee.lower()]
    if created_at: rows = [r for r in rows if str(r.get("created_at","")).lower() == created_at.lower()]
    return {"source_system": "ServiceNow", "source_system_id": "servicenow", "table": "incidents", "rows": rows[:max(1,min(limit,100))], "total": len(rows), "produces": ["incidents_records","incidents_summary"], "evidence": ["source_system_record"]}
...
def list_documents() -> dict[str, Any]:
    ...
    return {"documents": [{"id":"audit-finding-tracker-controls-playbook","title":"Audit Finding Tracker Controls Playbook","type":"policy","path":"documents/audit-finding-tracker-controls-playbook.md","wordCount":527}]}

def read_document(document_id: str) -> dict[str, Any]:
    """Read the full text of a document by its ID. Use for citing evidence."""
    docs = [{"id":"audit-finding-tracker-controls-playbook","path":"documents/audit-finding-tracker-controls-playbook.md","title":"Audit Finding Tracker Controls Playbook"}]
...
def search_documents(query: str = "") -> dict[str, Any]:
    ...
    results = []
    for doc_meta in [{"id":"audit-finding-tracker-controls-playbook","path":"documents/audit-finding-tracker-controls-playbook.md","title":"Audit Finding Tracker Controls Playbook"}]:
...
def lookup_audit_finding_tracker_controls_playbook(section_anchor: str = "", document_id: str = "") -> dict[str, Any]:
    """Look up sections of the Audit Finding Tracker Controls Playbook to cite in narrative output, escalation rationale, and audit evidence."""
    candidates = [d for d in _CONTRACT_DOCUMENTS if d.get("source_system_id") == "auditboard"]
    if document_id:
        candidates = [d for d in candidates if d.get("id") == document_id]
    if not candidates:
        return {"error": "no_document_for_source", "source_system_id": "auditboard", "tool_kind": "evidence_lookup"}
    best = None
    ...
    if best is None:
        return {"error": "documents_unavailable", "source_system_id": "auditboard", "tool_kind": "evidence_lookup"}
...
def action_auditboard_execute(target_id: str = "", rationale: str = "") -> dict[str, Any]:
    """Execute the execute step in AuditBoard after the agent has gathered evidence and validated escalation gates."""
    ...
    audit_trail = "action_auditboard_execute(target_id={target_id}, rationale={rationale})".format(target_id=target_id, rationale=rationale)
    action_id = _deterministic_id("ACTION_I", "action_auditboard_execute", target_id, rationale)
    audit_record_id = _deterministic_id("AUDIT_RE", "action_auditboard_execute", target_id, rationale)
    result = {
        "source_system_id": "auditboard",
        "tool_kind": "action",
        "status": "submitted",
    ...
    _append_action_event("action_auditboard_execute", result)
    return result
source_adapters_fixtures = [FunctionTool(func=list_systems), FunctionTool(func=describe_data_model), FunctionTool(func=query_auditboard_records), FunctionTool(func=query_auditboard_events), FunctionTool(func=query_auditboard_audit_trail), FunctionTool(func=query_servicenow_tickets), FunctionTool(func=query_servicenow_change_requests), FunctionTool(func=query_servicenow_incidents), FunctionTool(func=query_finance_3_records), FunctionTool(func=query_finance_3_events), FunctionTool(func=list_documents), FunctionTool(func=read_document), FunctionTool(func=search_documents), FunctionTool(func=lookup_audit_finding_tracker_controls_playbook), FunctionTool(func=action_auditboard_execute)]
...
_AGENT_ID = _os.environ.get("GE_AGENT_ID", "audit_finding_tracker")
# Registry server id this agent registers as (override via env if needed).
_MCP_SERVER = _os.environ.get("GE_MCP_SERVER", "audit-finding-tracker")
...
def _mcp_toolsets():
    ...
    toolsets = []
    for _name in [
        "mcpServers/audit-finding-tracker",
    ]:
        try:
