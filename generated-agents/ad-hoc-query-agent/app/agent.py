...
# Agent name : ad_hoc_query_agent_agent  (keep stable for evals / deployment metadata / GE registration)
...
_INSTRUCTION = """You are FP&A Analyst agent for the Ad-Hoc Query Agent workflow.
...
- query_bigquery_analytics_events [query, bigquery]: Retrieve analytics events from BigQuery for the Ad-Hoc Query Agent workflow.
- query_looker_dashboards [query, looker]: Retrieve dashboards from Looker for the Ad-Hoc Query Agent workflow.
- query_sap_s_4hana_transactions [query, sap_s_4hana]: Retrieve transactions from SAP S/4HANA for the Ad-Hoc Query Agent workflow.
- lookup_ad_hoc_query_agent_controls_playbook [evidence_lookup, bigquery]: Look up sections of the Ad-Hoc Query Agent Controls Playbook to cite in narrative output, escalation rationale, and audit evidence.
...
- For "Query response time moved from Hours-days (manual pull) toward < 30 seconds": cite bigquery.analytics_events, looker.dashboards (from bigquery, looker).
- For "Data accessibility moved from Requires SQL knowledge toward Natural language": cite bigquery.analytics_events, looker.dashboards (from bigquery, looker).

ESCALATION & REFUSAL TRIGGERS
- If Query response time regresses past the Hours-days (manual pull) baseline by more than 20%: escalate_to_human → FP&A Analyst. Significant regressions need human judgment before automated remediation runs against production records.
...
_SCENARIO_ID = "ad_hoc_query_agent"
_PRIMARY_OBJECTIVE = """Natural language interface makes financial data accessible to any business user. Gemini interprets ambiguous questions and returns answers with business context and citations. so the FP&A Analyst can move the Query response time KPI."""
_EXPECTED_TOOLS = ["query_bigquery_analytics_events","query_looker_dashboards","query_sap_s_4hana_transactions","lookup_ad_hoc_query_agent_controls_playbook"]
_WRITE_TOOLS = []
_REQUIRED_INPUTS_BY_TOOL = {"query_bigquery_analytics_events":["lookup_key","date_range"],"query_looker_dashboards":["lookup_key","date_range"],"query_sap_s_4hana_transactions":["lookup_key","date_range"],"lookup_ad_hoc_query_agent_controls_playbook":["section_anchor"]}
...
root_agent = Agent(
    name="ad_hoc_query_agent_agent",
    model="gemini-3.5-flash",
    description="Ad Hoc Query Agent: Natural language interface makes financial data accessible to any business user. Gemini interprets ambiguous questions and returns answers with business context and citations. so t",
