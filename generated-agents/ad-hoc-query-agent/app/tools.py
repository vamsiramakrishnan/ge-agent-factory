"""Auto-generated fixture-backed tools for: ad_hoc_query_agent"""
...
_CONTRACT_DOCUMENTS = [{"id":"ad-hoc-query-agent-controls-playbook","title":"Ad-Hoc Query Agent Controls Playbook","type":"policy","path":"documents/ad-hoc-query-agent-controls-playbook.md","source_system_id":"bigquery"}]
...
def list_systems() -> dict[str, Any]:
    ...
    return {"scenario": "ad_hoc_query_agent", "systems": [{"id":"bigquery","name":"BigQuery","owns":["analytics_events","historical_metrics","cached_aggregates"],"protocol":"BigQuery SQL","localBacking":["bigquery"],"toolNames":["query_bigquery_analytics_events","query_bigquery_historical_metrics","query_bigquery_cached_aggregates"],"evidence":["source_system_record","generated_audit_trail"]},{"id":"looker","name":"Looker","owns":["dashboards","explore_queries","metric_definitions"],"protocol":"LookerML","localBacking":["bigquery"],"toolNames":["query_looker_dashboards","query_looker_explore_queries","query_looker_metric_definitions"],"evidence":["source_system_record","generated_audit_trail"]},{"id":"sap_s_4hana","name":"SAP S/4HANA","owns":["transactions","journal_entries","master_data"],"protocol":"RFC/BAPI","localBacking":["alloydb"],"toolNames":["query_sap_s_4hana_transactions","query_sap_s_4hana_journal_entries","query_sap_s_4hana_master_data"],"evidence":["source_system_record","generated_audit_trail"]}], "tables": [{"name":"analytics_events","sourceSystem":"BigQuery","sourceSystemId":"bigquery"},{"name":"historical_metrics","sourceSystem":"BigQuery","sourceSystemId":"bigquery"},{"name":"cached_aggregates","sourceSystem":"BigQuery","sourceSystemId":"bigquery"},{"name":"dashboards","sourceSystem":"Looker","sourceSystemId":"looker"},{"name":"explore_queries","sourceSystem":"Looker","sourceSystemId":"looker"},{"name":"metric_definitions","sourceSystem":"Looker","sourceSystemId":"looker"},{"name":"transactions","sourceSystem":"SAP S/4HANA","sourceSystemId":"sap_s_4hana"},{"name":"journal_entries","sourceSystem":"SAP S/4HANA","sourceSystemId":"sap_s_4hana"},{"name":"master_data","sourceSystem":"SAP S/4HANA","sourceSystemId":"sap_s_4hana"}]}
...
def query_sap_s_4hana_transactions(lookup_key: str = "", date_range: str = "", id: str = "", posting_date: str = "", account: str = "", currency: str = "", description: str = "", status: str = "", limit: int = 20) -> dict[str, Any]:
    """Query SAP S/4HANA transactions. Columns: id, posting_date, account, amount, currency, description, status."""
    rows = _json("tables/transactions.json")
    if lookup_key:
        needle = lookup_key.lower()
        rows = [r for r in rows if any(needle in str(value).lower() for value in r.values())]
    if date_range:
    ...
    if status: rows = [r for r in rows if str(r.get("status","")).lower() == status.lower()]
    return {"source_system": "SAP S/4HANA", "source_system_id": "sap_s_4hana", "table": "transactions", "rows": rows[:max(1,min(limit,100))], "total": len(rows), "produces": ["transactions_records","transactions_summary"], "evidence": ["source_system_record"]}

def query_sap_s_4hana_journal_entries(id: str = "", posting_date: str = "", account: str = "", currency: str = "", description: str = "", status: str = "", limit: int = 20) -> dict[str, Any]:
    """Query SAP S/4HANA journal_entries. Columns: id, posting_date, account, amount, currency, description, status."""
    rows = _json("tables/journal_entries.json")
    if id: rows = [r for r in rows if str(r.get("id","")).lower() == id.lower()]
    if posting_date: rows = [r for r in rows if str(r.get("posting_date","")).lower() == posting_date.lower()]
    if account: rows = [r for r in rows if str(r.get("account","")).lower() == account.lower()]
    if currency: rows = [r for r in rows if str(r.get("currency","")).lower() == currency.lower()]
    if description: rows = [r for r in rows if str(r.get("description","")).lower() == description.lower()]
    if status: rows = [r for r in rows if str(r.get("status","")).lower() == status.lower()]
    return {"source_system": "SAP S/4HANA", "source_system_id": "sap_s_4hana", "table": "journal_entries", "rows": rows[:max(1,min(limit,100))], "total": len(rows), "produces": ["journal_entries_records","journal_entries_summary"], "evidence": ["source_system_record"]}

def query_sap_s_4hana_master_data(id: str = "", source_record_id: str = "", status: str = "", owner: str = "", created_at: str = "", notes: str = "", limit: int = 20) -> dict[str, Any]:
    """Query SAP S/4HANA master_data. Columns: id, source_record_id, status, owner, created_at, notes."""
    rows = _json("tables/master_data.json")
    if id: rows = [r for r in rows if str(r.get("id","")).lower() == id.lower()]
    if source_record_id: rows = [r for r in rows if str(r.get("source_record_id","")).lower() == source_record_id.lower()]
    if status: rows = [r for r in rows if str(r.get("status","")).lower() == status.lower()]
    if owner: rows = [r for r in rows if str(r.get("owner","")).lower() == owner.lower()]
    if created_at: rows = [r for r in rows if str(r.get("created_at","")).lower() == created_at.lower()]
    if notes: rows = [r for r in rows if str(r.get("notes","")).lower() == notes.lower()]
    return {"source_system": "SAP S/4HANA", "source_system_id": "sap_s_4hana", "table": "master_data", "rows": rows[:max(1,min(limit,100))], "total": len(rows), "produces": ["master_data_records","master_data_summary"], "evidence": ["source_system_record"]}

def list_documents() -> dict[str, Any]:
    """List all available evidence documents and their metadata."""
    return {"documents": [{"id":"ad-hoc-query-agent-controls-playbook","title":"Ad-Hoc Query Agent Controls Playbook","type":"policy","path":"documents/ad-hoc-query-agent-controls-playbook.md","wordCount":676}]}

def read_document(document_id: str) -> dict[str, Any]:
    """Read the full text of a document by its ID. Use for citing evidence."""
    docs = [{"id":"ad-hoc-query-agent-controls-playbook","path":"documents/ad-hoc-query-agent-controls-playbook.md","title":"Ad-Hoc Query Agent Controls Playbook"}]
...
def search_documents(query: str = "") -> dict[str, Any]:
    ...
    results = []
    for doc_meta in [{"id":"ad-hoc-query-agent-controls-playbook","path":"documents/ad-hoc-query-agent-controls-playbook.md","title":"Ad-Hoc Query Agent Controls Playbook"}]:
...
def lookup_ad_hoc_query_agent_controls_playbook(section_anchor: str = "", document_id: str = "") -> dict[str, Any]:
    """Look up sections of the Ad-Hoc Query Agent Controls Playbook to cite in narrative output, escalation rationale, and audit evidence."""
...
source_adapters_fixtures = [FunctionTool(func=list_systems), FunctionTool(func=describe_data_model), FunctionTool(func=query_bigquery_analytics_events), FunctionTool(func=query_bigquery_historical_metrics), FunctionTool(func=query_bigquery_cached_aggregates), FunctionTool(func=query_looker_dashboards), FunctionTool(func=query_looker_explore_queries), FunctionTool(func=query_looker_metric_definitions), FunctionTool(func=query_sap_s_4hana_transactions), FunctionTool(func=query_sap_s_4hana_journal_entries), FunctionTool(func=query_sap_s_4hana_master_data), FunctionTool(func=list_documents), FunctionTool(func=read_document), FunctionTool(func=search_documents), FunctionTool(func=lookup_ad_hoc_query_agent_controls_playbook)]
...
_AGENT_ID = _os.environ.get("GE_AGENT_ID", "ad_hoc_query_agent")
# Registry server id this agent registers as (override via env if needed).
_MCP_SERVER = _os.environ.get("GE_MCP_SERVER", "ad-hoc-query-agent")
