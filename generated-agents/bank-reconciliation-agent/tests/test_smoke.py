"""Auto-generated smoke tests for: bank_reconciliation_agent"""
import json, csv, os
...
class TestFixtures:
    def test_manifest(self):
        ...
        assert m["id"] == "bank_reconciliation_agent"

    def test_cash_positions_exists(self):
        p = FIX / "tables/cash_positions.json"
        assert p.exists()
        data = json.loads(p.read_text()) if p.suffix == ".json" else list(csv.DictReader(p.open(newline="")))
        assert len(data) > 0

    def test_bank_transactions_exists(self):
        p = FIX / "tables/bank_transactions.json"
        assert p.exists()
        data = json.loads(p.read_text()) if p.suffix == ".json" else list(csv.DictReader(p.open(newline="")))
        assert len(data) > 0

    def test_forecast_inputs_exists(self):
        p = FIX / "tables/forecast_inputs.json"
        assert p.exists()
        data = json.loads(p.read_text()) if p.suffix == ".json" else list(csv.DictReader(p.open(newline="")))
        assert len(data) > 0
...
class TestTools:
    ...
    def test_query_kyriba_cash_positions(self):
        from app.tools import query_kyriba_cash_positions
        r = query_kyriba_cash_positions(limit=5)
        assert r["total"] > 0
        assert r.get("source_system")

    def test_query_kyriba_bank_transactions(self):
        from app.tools import query_kyriba_bank_transactions
        r = query_kyriba_bank_transactions(limit=5)
        assert r["total"] > 0
        assert r.get("source_system")

    def test_query_kyriba_forecast_inputs(self):
        from app.tools import query_kyriba_forecast_inputs
        r = query_kyriba_forecast_inputs(limit=5)
        assert r["total"] > 0
        assert r.get("source_system")
...
class TestBehaviorContract:
    def test_contract_tools_importable(self):
        ...
        for fn in ["query_kyriba_cash_positions","query_sap_s_4hana_fi_gl_entries","query_bigquery_analytics_events","lookup_bank_reconciliation_agent_controls_playbook","action_kyriba_match"]:
    ...
    def test_write_tools_emit_action_events(self, tmp_path, monkeypatch):
        ...
        from app import tools as tools_module
        result = getattr(tools_module, "action_kyriba_match")(**{"target_id":"test_target_id","rationale":"test_rationale"})
        assert result.get("audit_trail")
        event_path = tmp_path / "action-events.jsonl"
        assert event_path.exists()
        assert "action_kyriba_match" in event_path.read_text()
