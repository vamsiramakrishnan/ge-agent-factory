"""Auto-generated fixture-backed tools for: bank_reconciliation_agent"""
...
_CONTRACT_DOCUMENTS = [{"id":"bank-reconciliation-agent-controls-playbook","title":"Bank Reconciliation Agent Controls Playbook","type":"policy","path":"documents/bank-reconciliation-agent-controls-playbook.md","source_system_id":"bigquery"}]
...
def list_systems() -> dict[str, Any]:
    ...
    return {"scenario": "bank_reconciliation_agent", "systems": [{"id":"kyriba","name":"Kyriba","owns":["cash_positions","bank_transactions","forecast_inputs"],"protocol":"REST API","localBacking":["alloydb"],"toolNames":["query_kyriba_cash_positions","query_kyriba_bank_transactions","query_kyriba_forecast_inputs"],"evidence":["source_system_record","generated_audit_trail"]},{"id":"sap_s_4hana_fi","name":"SAP S/4HANA FI","owns":["gl_entries","subledger_balances","open_items"],"protocol":"RFC/BAPI","localBacking":["alloydb"],"toolNames":["query_sap_s_4hana_fi_gl_entries","query_sap_s_4hana_fi_subledger_balances","query_sap_s_4hana_fi_open_items"],"evidence":["source_system_record","generated_audit_trail"]},{"id":"bigquery","name":"BigQuery","owns":["analytics_events","historical_metrics","cached_aggregates"],"protocol":"BigQuery SQL","localBacking":["bigquery"],"toolNames":["query_bigquery_analytics_events","query_bigquery_historical_metrics","query_bigquery_cached_aggregates"],"evidence":["source_system_record","generated_audit_trail"]}], "tables": [{"name":"cash_positions","sourceSystem":"Kyriba","sourceSystemId":"kyriba"},{"name":"bank_transactions","sourceSystem":"Kyriba","sourceSystemId":"kyriba"},{"name":"forecast_inputs","sourceSystem":"Kyriba","sourceSystemId":"kyriba"},{"name":"gl_entries","sourceSystem":"SAP S/4HANA FI","sourceSystemId":"sap_s_4hana_fi"},{"name":"subledger_balances","sourceSystem":"SAP S/4HANA FI","sourceSystemId":"sap_s_4hana_fi"},{"name":"open_items","sourceSystem":"SAP S/4HANA FI","sourceSystemId":"sap_s_4hana_fi"},{"name":"analytics_events","sourceSystem":"BigQuery","sourceSystemId":"bigquery"},{"name":"historical_metrics","sourceSystem":"BigQuery","sourceSystemId":"bigquery"},{"name":"cached_aggregates","sourceSystem":"BigQuery","sourceSystemId":"bigquery"}]}

def query_kyriba_cash_positions(lookup_key: str = "", date_range: str = "", id: str = "", source_record_id: str = "", status: str = "", owner: str = "", created_at: str = "", notes: str = "", limit: int = 20) -> dict[str, Any]:
    """Query Kyriba cash_positions. Columns: id, source_record_id, status, owner, created_at, notes."""
    rows = _json("tables/cash_positions.json")
    if lookup_key:
    ...
    return {"source_system": "Kyriba", "source_system_id": "kyriba", "table": "cash_positions", "rows": rows[:max(1,min(limit,100))], "total": len(rows), "produces": ["cash_positions_records","cash_positions_summary"], "evidence": ["source_system_record"]}

def query_kyriba_bank_transactions(id: str = "", posting_date: str = "", account: str = "", currency: str = "", description: str = "", status: str = "", limit: int = 20) -> dict[str, Any]:
    """Query Kyriba bank_transactions. Columns: id, posting_date, account, amount, currency, description, status."""
    rows = _json("tables/bank_transactions.json")
    if id: rows = [r for r in rows if str(r.get("id","")).lower() == id.lower()]
    ...
    if status: rows = [r for r in rows if str(r.get("status","")).lower() == status.lower()]
    return {"source_system": "Kyriba", "source_system_id": "kyriba", "table": "bank_transactions", "rows": rows[:max(1,min(limit,100))], "total": len(rows), "produces": ["bank_transactions_records","bank_transactions_summary"], "evidence": ["source_system_record"]}

def query_kyriba_forecast_inputs(id: str = "", cost_center: str = "", period: str = "", scenario: str = "", limit: int = 20) -> dict[str, Any]:
    """Query Kyriba forecast_inputs. Columns: id, cost_center, period, budget_amount, actual_amount, variance_pct, scenario."""
    rows = _json("tables/forecast_inputs.json")
    if id: rows = [r for r in rows if str(r.get("id","")).lower() == id.lower()]
    if cost_center: rows = [r for r in rows if str(r.get("cost_center","")).lower() == cost_center.lower()]
    if period: rows = [r for r in rows if str(r.get("period","")).lower() == period.lower()]
    if scenario: rows = [r for r in rows if str(r.get("scenario","")).lower() == scenario.lower()]
    return {"source_system": "Kyriba", "source_system_id": "kyriba", "table": "forecast_inputs", "rows": rows[:max(1,min(limit,100))], "total": len(rows), "produces": ["forecast_inputs_records","forecast_inputs_summary"], "evidence": ["source_system_record"]}
...
def list_documents() -> dict[str, Any]:
    ...
    return {"documents": [{"id":"bank-reconciliation-agent-controls-playbook","title":"Bank Reconciliation Agent Controls Playbook","type":"policy","path":"documents/bank-reconciliation-agent-controls-playbook.md","wordCount":555}]}

def read_document(document_id: str) -> dict[str, Any]:
    """Read the full text of a document by its ID. Use for citing evidence."""
    docs = [{"id":"bank-reconciliation-agent-controls-playbook","path":"documents/bank-reconciliation-agent-controls-playbook.md","title":"Bank Reconciliation Agent Controls Playbook"}]
...
def search_documents(query: str = "") -> dict[str, Any]:
    ...
    results = []
    for doc_meta in [{"id":"bank-reconciliation-agent-controls-playbook","path":"documents/bank-reconciliation-agent-controls-playbook.md","title":"Bank Reconciliation Agent Controls Playbook"}]:
...
def lookup_bank_reconciliation_agent_controls_playbook(section_anchor: str = "", document_id: str = "") -> dict[str, Any]:
    """Look up sections of the Bank Reconciliation Agent Controls Playbook to cite in narrative output, escalation rationale, and audit evidence."""
...
def action_kyriba_match(target_id: str = "", rationale: str = "") -> dict[str, Any]:
    ...
    audit_trail = "action_kyriba_match(target_id={target_id}, rationale={rationale})".format(target_id=target_id, rationale=rationale)
    action_id = _deterministic_id("ACTION_I", "action_kyriba_match", target_id, rationale)
    audit_record_id = _deterministic_id("AUDIT_RE", "action_kyriba_match", target_id, rationale)
    result = {
        "source_system_id": "kyriba",
        "tool_kind": "action",
        "status": "submitted",
        "action_id": action_id,
...
_AGENT_ID = _os.environ.get("GE_AGENT_ID", "bank_reconciliation_agent")
# Registry server id this agent registers as (override via env if needed).
_MCP_SERVER = _os.environ.get("GE_MCP_SERVER", "bank-reconciliation-agent")
...
def _mcp_toolsets():
    ...
    toolsets = []
    for _name in [
        "mcpServers/bank-reconciliation-agent",
    ]:
        try:
