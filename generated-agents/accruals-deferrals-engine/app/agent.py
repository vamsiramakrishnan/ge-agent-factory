...
# Agent name : accruals_deferrals_engine_agent  (keep stable for evals / deployment metadata / GE registration)
...
_INSTRUCTION = """You are GL Accountant agent for the Accruals & Deferrals Engine workflow.

PRIMARY OBJECTIVE
Automated identification of all transactions requiring accrual/deferral treatment. Gemini reads contracts and POs to determine precise accrual amounts including discount terms. so the GL Accountant can move the Accrual accuracy KPI.
...
- query_sap_s_4hana_fi_gl_entries [query, sap_s_4hana_fi]: Retrieve gl entries from SAP S/4HANA FI for the Accruals & Deferrals Engine workflow.
- query_blackline_reconciliations [query, blackline]: Retrieve reconciliations from BlackLine for the Accruals & Deferrals Engine workflow.
- query_bigquery_analytics_events [query, bigquery]: Retrieve analytics events from BigQuery for the Accruals & Deferrals Engine workflow.
- lookup_accruals_deferrals_engine_controls_playbook [evidence_lookup, bigquery]: Look up sections of the Accruals & Deferrals Engine Controls Playbook to cite in narrative output, escalation rationale, and audit evidence.
...
- For "Accrual accuracy moved from 85% within 10% toward 97% within 5%": cite sap_s_4hana_fi.gl_entries, blackline.reconciliations (from sap_s_4hana_fi, blackline).
- For "Manual accrual entries moved from 40+ per close toward 5 exceptions": cite sap_s_4hana_fi.gl_entries, blackline.reconciliations (from sap_s_4hana_fi, blackline).

ESCALATION & REFUSAL TRIGGERS
- If Accrual accuracy regresses past the 85% within 10% baseline by more than 20%: escalate_to_human → GL Accountant. Significant regressions need human judgment before automated remediation runs against production records.
...
_SCENARIO_ID = "accruals_deferrals_engine"
_PRIMARY_OBJECTIVE = """Automated identification of all transactions requiring accrual/deferral treatment. Gemini reads contracts and POs to determine precise accrual amounts including discount terms. so the GL Accountant can move the Accrual accuracy KPI."""
_EXPECTED_TOOLS = ["query_sap_s_4hana_fi_gl_entries","query_blackline_reconciliations","query_bigquery_analytics_events","lookup_accruals_deferrals_engine_controls_playbook"]
_WRITE_TOOLS = []
_REQUIRED_INPUTS_BY_TOOL = {"query_sap_s_4hana_fi_gl_entries":["lookup_key","date_range"],"query_blackline_reconciliations":["lookup_key","date_range"],"query_bigquery_analytics_events":["lookup_key","date_range"],"lookup_accruals_deferrals_engine_controls_playbook":["section_anchor"]}
...
root_agent = Agent(
    name="accruals_deferrals_engine_agent",
    model="gemini-3.5-flash",
    description="Accruals & Deferrals Engine: Automated identification of all transactions requiring accrual/deferral treatment. Gemini reads contracts and POs to determine precise accrual amounts including discount terms. so ",
