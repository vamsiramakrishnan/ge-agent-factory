...
# Agent name : audit_report_generator_agent  (keep stable for evals / deployment metadata / GE registration)
...
_INSTRUCTION = """You are Chief Audit Executive agent for the Audit Report Generator workflow.
...
- query_auditboard_auditboard_records [query, auditboard]: Retrieve auditboard records from AuditBoard for the Audit Report Generator workflow.
- query_google_docs_documents [query, google_docs]: Retrieve documents from Google Docs for the Audit Report Generator workflow.
- query_finance_3_finance_3_records [query, finance_3]: Retrieve finance 3 records from FINANCE 3 for the Audit Report Generator workflow.
- lookup_audit_report_generator_controls_playbook [evidence_lookup, google_docs]: Look up sections of the Audit Report Generator Controls Playbook to cite in narrative output, escalation rationale, and audit evidence.
- action_auditboard_recommend [action, auditboard]: Execute the recommend step in AuditBoard after the agent has gathered evidence and validated escalation gates.
...
- For "Report drafting time moved from 2-3 weeks toward 2-3 days": cite auditboard.auditboard_records, google_docs.documents (from auditboard, google_docs).
- For "Finding consistency moved from Varies by auditor toward Standardized format": cite auditboard.auditboard_records, google_docs.documents (from auditboard, google_docs).

ESCALATION & REFUSAL TRIGGERS
- If Report drafting time regresses past the 2-3 weeks baseline by more than 20%: escalate_to_human → Chief Audit Executive. Significant regressions need human judgment before automated remediation runs against production records.
...
- If Proposed recommend action lacks supporting evidence from at least two systems: refuse. Single-system evidence is insufficient to authorize external state changes without manual review.
...
_SCENARIO_ID = "audit_report_generator"
_PRIMARY_OBJECTIVE = """Gemini drafts comprehensive audit reports with executive summary, root cause analysis, and recommendations. Automatic tone adaptation — risk-focused summary for audit committee, remediation guidance for management. so the Chief Audit Executive can move the Report drafting time KPI."""
_EXPECTED_TOOLS = ["query_auditboard_records","query_google_docs_documents","query_finance_3_records","lookup_audit_report_generator_controls_playbook","action_auditboard_recommend"]
_WRITE_TOOLS = ["action_auditboard_recommend"]
_REQUIRED_INPUTS_BY_TOOL = {"query_auditboard_auditboard_records":["lookup_key","date_range"],"query_google_docs_documents":["lookup_key","date_range"],"query_finance_3_finance_3_records":["lookup_key","date_range"],"lookup_audit_report_generator_controls_playbook":["section_anchor"],"action_auditboard_recommend":["target_id","rationale"]}
_EVIDENCE_MIN_SYSTEMS_BY_TOOL = {"action_auditboard_recommend":2}
...
root_agent = Agent(
    name="audit_report_generator_agent",
    model="gemini-3.5-flash",
    description="Audit Report Generator: Gemini drafts comprehensive audit reports with executive summary, root cause analysis, and recommendations. Automatic tone adaptation — risk-focused summary for audit committee, re",
