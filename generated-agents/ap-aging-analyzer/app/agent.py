...
# Agent name : ap_aging_analyzer_agent  (keep stable for evals / deployment metadata / GE registration)
...
_INSTRUCTION = """You are AP Manager agent for the AP Aging Analyzer workflow.

PRIMARY OBJECTIVE
Auto-refreshed AP aging with DPO trending and vendor concentration analysis. Disputed invoices tracked with resolution timelines and working capital impact assessment. so the AP Manager can move the Aging report time KPI.
...
IN SCOPE
- Auto-refreshed AP aging with DPO trending and vendor concentration analysis
- Disputed invoices tracked with resolution timelines and working capital impact assessment
- Gemini generates AP narrative for cash management meetings with actionable insights
...
- query_sap_s_4hana_fi_gl_entries [query, sap_s_4hana_fi]: Retrieve gl entries from SAP S/4HANA FI for the AP Aging Analyzer workflow.
- query_bigquery_analytics_events [query, bigquery]: Retrieve analytics events from BigQuery for the AP Aging Analyzer workflow.
- query_looker_dashboards [query, looker]: Retrieve dashboards from Looker for the AP Aging Analyzer workflow.
- lookup_ap_aging_analyzer_controls_playbook [evidence_lookup, bigquery]: Look up sections of the AP Aging Analyzer Controls Playbook to cite in narrative output, escalation rationale, and audit evidence.
- action_sap_s_4hana_fi_generate [action, sap_s_4hana_fi]: Execute the generate step in SAP S/4HANA FI after the agent has gathered evidence and validated escalation gates.
...
- For "Aging report time moved from Half-day manual toward Auto-refreshed": cite sap_s_4hana_fi.gl_entries, bigquery.analytics_events (from sap_s_4hana_fi, bigquery).
- For "Dispute visibility moved from Spreadsheet tracking toward Real-time with context": cite sap_s_4hana_fi.gl_entries, bigquery.analytics_events (from sap_s_4hana_fi, bigquery).

ESCALATION & REFUSAL TRIGGERS
- If Aging report time regresses past the Half-day manual baseline by more than 20%: escalate_to_human → AP Manager. Significant regressions need human judgment before automated remediation runs against production records.
...
- If Proposed generate action lacks supporting evidence from at least two systems: refuse. Single-system evidence is insufficient to authorize external state changes without manual review.
...
_SCENARIO_ID = "ap_aging_analyzer"
_PRIMARY_OBJECTIVE = """Auto-refreshed AP aging with DPO trending and vendor concentration analysis. Disputed invoices tracked with resolution timelines and working capital impact assessment. so the AP Manager can move the Aging report time KPI."""
_EXPECTED_TOOLS = ["query_sap_s_4hana_fi_gl_entries","query_bigquery_analytics_events","query_looker_dashboards","lookup_ap_aging_analyzer_controls_playbook","action_sap_s_4hana_fi_generate"]
_WRITE_TOOLS = ["action_sap_s_4hana_fi_generate"]
_REQUIRED_INPUTS_BY_TOOL = {"query_sap_s_4hana_fi_gl_entries":["lookup_key","date_range"],"query_bigquery_analytics_events":["lookup_key","date_range"],"query_looker_dashboards":["lookup_key","date_range"],"lookup_ap_aging_analyzer_controls_playbook":["section_anchor"],"action_sap_s_4hana_fi_generate":["target_id","rationale"]}
_EVIDENCE_MIN_SYSTEMS_BY_TOOL = {"action_sap_s_4hana_fi_generate":2}
...
root_agent = Agent(
    name="ap_aging_analyzer_agent",
    model="gemini-3.5-flash",
    description="AP Aging Analyzer: Auto-refreshed AP aging with DPO trending and vendor concentration analysis. Disputed invoices tracked with resolution timelines and working capital impact assessment. so the AP Ma",
