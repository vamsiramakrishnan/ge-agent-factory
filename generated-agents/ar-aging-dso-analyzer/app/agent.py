...
# Agent name : ar_aging_dso_analyzer_agent  (keep stable for evals / deployment metadata / GE registration)
...
_INSTRUCTION = """You are AR Manager agent for the AR Aging & DSO Analyzer workflow.
...
- query_sap_s_4hana_fi_gl_entries [query, sap_s_4hana_fi]: Retrieve gl entries from SAP S/4HANA FI for the AR Aging & DSO Analyzer workflow.
- query_bigquery_analytics_events [query, bigquery]: Retrieve analytics events from BigQuery for the AR Aging & DSO Analyzer workflow.
- query_looker_dashboards [query, looker]: Retrieve dashboards from Looker for the AR Aging & DSO Analyzer workflow.
- lookup_ar_aging_dso_analyzer_controls_playbook [evidence_lookup, bigquery]: Look up sections of the AR Aging & DSO Analyzer Controls Playbook to cite in narrative output, escalation rationale, and audit evidence.
...
- For "Reporting cycle moved from Manual weekly toward Auto-refreshed daily": cite sap_s_4hana_fi.gl_entries, bigquery.analytics_events (from sap_s_4hana_fi, bigquery).
- For "Root cause visibility moved from Top-line DSO only toward Segment-level with narrative": cite sap_s_4hana_fi.gl_entries, bigquery.analytics_events (from sap_s_4hana_fi, bigquery).

ESCALATION & REFUSAL TRIGGERS
- If Reporting cycle regresses past the Manual weekly baseline by more than 20%: escalate_to_human → AR Manager. Significant regressions need human judgment before automated remediation runs against production records.
...
_SCENARIO_ID = "ar_aging_dso_analyzer"
_PRIMARY_OBJECTIVE = """Automated segment-level DSO analysis reveals that a 5-day DSO increase is driven entirely by 2 government contracts, not a systemic issue. Gemini generates narrative context that transforms numbers into actionable intelligence for the AR Manager. so the AR Manager can move the Reporting cycle KPI."""
_EXPECTED_TOOLS = ["query_sap_s_4hana_fi_gl_entries","query_bigquery_analytics_events","query_looker_dashboards","lookup_ar_aging_dso_analyzer_controls_playbook","action_sap_s_4hana_fi_generate"]
_WRITE_TOOLS = ["action_sap_s_4hana_fi_generate"]
_REQUIRED_INPUTS_BY_TOOL = {"query_sap_s_4hana_fi_gl_entries":["lookup_key","date_range"],"query_bigquery_analytics_events":["lookup_key","date_range"],"query_looker_dashboards":["lookup_key","date_range"],"lookup_ar_aging_dso_analyzer_controls_playbook":["section_anchor"],"action_sap_s_4hana_fi_generate":["target_id","rationale"]}
...
root_agent = Agent(
    name="ar_aging_dso_analyzer_agent",
    model="gemini-3.5-flash",
    description="AR Aging & DSO Analyzer: Automated segment-level DSO analysis reveals that a 5-day DSO increase is driven entirely by 2 government contracts, not a systemic issue. Gemini generates narrative context that t",
