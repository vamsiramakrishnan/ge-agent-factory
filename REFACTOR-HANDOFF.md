...
2. **`TIMEOUTS` taxonomy** (factory.mjs, ~24 bare timeout literals). **DECIDED (2026-07-05): won't-do-for-now — this is a deliberate call, not an open question.** The **same value serves different operations** (`60000` = gcloud-mutate / agents-info-read / scaffold / IAM-grant), so a purpose-name lies at half its sites and a per-value bucket (e.g. `TIMEOUTS.GCLOUD_MUTATE`) is just the number renamed — worse than the bare literal, since it *asserts* a single purpose for a value four unrelated call sites share. Building the real fix means a genuine timeout-policy design (per-operation budgets, not per-value buckets), which is separate, larger-effort work than this decision itself. Named unlock conditions — build the taxonomy when **either** holds: (a) a concrete incident or operator complaint ties back to an undifferentiated timeout value (e.g. someone needs to tune one operation's budget without affecting the other three that happen to share its literal today); or (b) `factory.mjs`'s command surface gets restructured (by a future decomposition wave) enough that the ~24 literals naturally separate into fewer, purpose-distinct call sites where a per-purpose bucket stops "lying." Absent either, the bare literals stay — a future worker who disagrees and wants to build the taxonomy now may, but it's a deliberate choice against these stated conditions, not a default.
