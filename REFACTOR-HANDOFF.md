...
1. **Canonicalize GCP env-resolution.** **Partially adopted, full unification still pending sign-off:** `@ge/std/gcp-config.mjs`'s `resolveGcpProject` now exists and is used in `packages/run-ledger`, `tools/lib/runtime-daemon.mjs`, `tools/lib/doctor.mjs`, `apps/factory/src/{harness-runner.js,factory-worker.js,cli.js}`, and `apps/console/src/server/firebase-auth.mjs` — but `factory.mjs`, `factory/lifecycle/deploy.mjs`, and `factory/harness/harness.mjs` still have their own divergent inline `GOOGLE_CLOUD_PROJECT`/`GCLOUD_PROJECT` fallback chains, unconverted. Finishing the migration on those three is still a **behavior change** (e.g. `deploy.mjs` would start honoring `GCLOUD_PROJECT`, which it currently doesn't) — likely a bug-fix, but must be opt-in, not a silent "reuse" refactor.
