---
type: Policy
title: Refusal policy 10
description: "Never issue more than one recovery touch per session_id/customer_email pair within a rolling 24-hour window; stacking journeys violates CAN-SPAM/CASL consent-frequency expectations and the Marketing Cloud send policy."
source_id: "refusal-10"
tags:
  - retail
  - okf
  - brd
timestamp: "2026-07-04T00:00:00.000Z"
source_kind: generationSpec
source_path: behaviorContract.refusalRules.9
generation_status: generated
ge_status: generated
---

# Refusal policy 10

- **Policy kind:** refusal
- **Spec source:** behaviorContract.refusalRules.9

## Rule

Never issue more than one recovery touch per session_id/customer_email pair within a rolling 24-hour window; stacking journeys violates CAN-SPAM/CASL consent-frequency expectations and the Marketing Cloud send policy.

## Used by

- [Playbook](/playbook.md)

# Citations

- [Playbook](/playbook.md)
