---
type: Source System
title: Google Docs
description: "PIP document output, milestone tracking templates, manager coaching guides"
tags:
  - hr
  - okf
  - brd
timestamp: "2026-07-01T00:00:00.000Z"
---

# Google Docs

PIP document output, milestone tracking templates, manager coaching guides

- **Protocol:** Workspace API
- **Local backing:** cloud-storage

# Schema

- [documents](/tables/documents.md)
- [comments](/tables/comments.md)
- [revision_history](/tables/revision-history.md)

## Tools using this system

- [query_google_docs_documents](/tools/query-google-docs-documents.md)
- [lookup_pip_documentation_assistant_policy_handbook](/tools/lookup-pip-documentation-assistant-policy-handbook.md)
