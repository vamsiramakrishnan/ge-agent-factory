---
type: Agent Tool
title: query_cdp_cdp_records
description: "Retrieve cdp records from CDP for the ESG & Sustainability Reporter workflow."
tags:
  - finance
  - okf
  - brd
timestamp: "2026-07-01T00:00:00.000Z"
source_kind: generationSpec
source_path: behaviorContract.toolIntents
generation_status: generated
ge_status: generated
---

# query_cdp_cdp_records

Retrieve cdp records from CDP for the ESG & Sustainability Reporter workflow.

- **Kind:** query
- **Source system:** [CDP](/systems/cdp.md)

## Inputs

- lookup_key
- date_range

## Outputs

- cdp_records_records
- cdp_records_summary

## Side Effects

- No mutation is declared by the spec for this tool.

## Idempotency

No idempotency key declared; no mutation is declared by the spec for this tool.

## Confirmation

No write confirmation policy is derivable from the spec.

## Permissions

No explicit permission scopes declared; source-system access is tied to [CDP](/systems/cdp.md).

## Failure Modes

No explicit failure modes are declared in the spec; rely on refusal/escalation policies for unsafe or incomplete evidence.

## Used By

_Not bound to a workflow stage._

## Evals

- [Run the ESG & Sustainability Reporter workflow for the current period. Cite the relevant source-system evidence and surface any escalations required.](/tests/esg-sustainability-reporter-end-to-end.md)

## Evidence emitted

- source_system_record

## Required inputs

- lookup_key
- date_range

## Produces

- cdp_records_records
- cdp_records_summary

# Examples

```
query_cdp_cdp_records(lookup_key=<lookup_key>, date_range=<date_range>)
```

# Citations

- [CDP](/systems/cdp.md)
