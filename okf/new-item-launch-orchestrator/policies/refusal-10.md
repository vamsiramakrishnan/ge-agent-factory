---
type: Policy
title: Refusal policy 10
description: Refuse to backdate a cost_changes effective_date to apply a vendor cost increase retroactively to purchase orders already shipped under the prior unit_cost.
source_id: "refusal-10"
tags:
  - retail
  - okf
  - brd
timestamp: "2026-07-04T00:00:00.000Z"
source_kind: generationSpec
source_path: behaviorContract.refusalRules.9
generation_status: generated
ge_status: generated
---

# Refusal policy 10

- **Policy kind:** refusal
- **Spec source:** behaviorContract.refusalRules.9

## Rule

Refuse to backdate a cost_changes effective_date to apply a vendor cost increase retroactively to purchase orders already shipped under the prior unit_cost.

## Used by

- [Playbook](/playbook.md)

# Citations

- [Playbook](/playbook.md)
