---
type: Agent Tool
title: lookup_goods_receipt_service_entry_validator_policy_guide
description: "Look up sections of the Goods Receipt & Service Entry Validator Procurement Policy Guide to cite in narrative output, escalation rationale, and audit evidence."
tags:
  - procurement
  - okf
  - brd
timestamp: "2026-07-01T00:00:00.000Z"
source_kind: generationSpec
source_path: behaviorContract.toolIntents
generation_status: generated
ge_status: generated
---

# lookup_goods_receipt_service_entry_validator_policy_guide

Look up sections of the Goods Receipt & Service Entry Validator Procurement Policy Guide to cite in narrative output, escalation rationale, and audit evidence.

- **Kind:** evidence_lookup
- **Source system:** [SAP S/4HANA MM (MIGO)](/systems/sap-s-4hana-mm-migo.md)

## Inputs

- section_anchor

## Outputs

- document_section
- citation_anchor

## Side Effects

- No mutation is declared by the spec for this tool.

## Idempotency

No idempotency key declared; no mutation is declared by the spec for this tool.

## Confirmation

No write confirmation policy is derivable from the spec.

## Permissions

No explicit permission scopes declared; source-system access is tied to [SAP S/4HANA MM (MIGO)](/systems/sap-s-4hana-mm-migo.md).

## Failure Modes

No explicit failure modes are declared in the spec; rely on refusal/escalation policies for unsafe or incomplete evidence.

## Used By

- [receipt_data_ingestion](/workflow/receipt-data-ingestion.md)
- [service_entry_validation](/workflow/service-entry-validation.md)
- [receipt_confirmation_downstream_trigger](/workflow/receipt-confirmation-downstream-trigger.md)

## Evals

- [Run the Goods Receipt & Service Entry Validator workflow for the current period. Cite the relevant source-system evidence and surface any escalations required.](/tests/goods-receipt-service-entry-validator-end-to-end.md)

## Evidence emitted

- document_reference

## Required inputs

- section_anchor

## Produces

- document_section
- citation_anchor

# Examples

```
lookup_goods_receipt_service_entry_validator_policy_guide(section_anchor=<section_anchor>)
```

# Citations

- [SAP S/4HANA MM (MIGO)](/systems/sap-s-4hana-mm-migo.md)
