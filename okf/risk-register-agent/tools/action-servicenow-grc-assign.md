---
type: Agent Tool
title: action_servicenow_grc_assign
description: Execute the assign step in ServiceNow GRC after the agent has gathered evidence and validated escalation gates.
tags:
  - it
  - okf
  - brd
timestamp: "2026-07-01T00:00:00.000Z"
source_kind: generationSpec
source_path: behaviorContract.toolIntents
generation_status: generated
ge_status: generated
---

# action_servicenow_grc_assign

Execute the assign step in ServiceNow GRC after the agent has gathered evidence and validated escalation gates.

- **Kind:** action
- **Source system:** [ServiceNow GRC](/systems/servicenow-grc.md)
- **API:** POST /api/servicenow_grc/assign

## Inputs

- target_id
- rationale

## Outputs

- action_id
- audit_record_id

## Side Effects

- May change ServiceNow GRC state because the spec classifies it as action.

## Idempotency

Declared idempotency key: target_id+rationale.

## Confirmation

- [Confirmation policy — action_servicenow_grc_assign](/policies/confirmation-action-servicenow-grc-assign.md)

## Permissions

No explicit permission scopes declared; source-system access is tied to [ServiceNow GRC](/systems/servicenow-grc.md).

## Failure Modes

No explicit failure modes are declared in the spec; rely on refusal/escalation policies for unsafe or incomplete evidence.

## Used By

- [register_update](/workflow/register-update.md)

## Evals

- [Run the Risk Register Agent workflow for the current period. Cite the relevant source-system evidence and surface any escalations required.](/tests/risk-register-agent-end-to-end.md)

## Evidence emitted

- api_response
- generated_audit_trail

## Required inputs

- target_id
- rationale

## Produces

- action_id
- audit_record_id

# Examples

```
action_servicenow_grc_assign(target_id=<target_id>, rationale=<rationale>)
```

# Citations

- [ServiceNow GRC](/systems/servicenow-grc.md)
- [Confirmation policy — action_servicenow_grc_assign](/policies/confirmation-action-servicenow-grc-assign.md)
- [Idempotency policy — action_servicenow_grc_assign](/policies/idempotency-action-servicenow-grc-assign.md)
