---
type: Agent Tool
title: query_pagerduty_incidents
description: Retrieve incidents from PagerDuty for the Security Incident Responder workflow.
tags:
  - it
  - okf
  - brd
timestamp: "2026-07-01T00:00:00.000Z"
source_kind: generationSpec
source_path: behaviorContract.toolIntents
generation_status: generated
ge_status: generated
---

# query_pagerduty_incidents

Retrieve incidents from PagerDuty for the Security Incident Responder workflow.

- **Kind:** query
- **Source system:** [PagerDuty](/systems/pagerduty.md)

## Inputs

- lookup_key
- date_range

## Outputs

- incidents_records
- incidents_summary

## Side Effects

- No mutation is declared by the spec for this tool.

## Idempotency

No idempotency key declared; no mutation is declared by the spec for this tool.

## Confirmation

No write confirmation policy is derivable from the spec.

## Permissions

No explicit permission scopes declared; source-system access is tied to [PagerDuty](/systems/pagerduty.md).

## Failure Modes

No explicit failure modes are declared in the spec; rely on refusal/escalation policies for unsafe or incomplete evidence.

## Used By

- [incident_classification_scoping](/workflow/incident-classification-scoping.md)

## Evals

- [Run the Security Incident Responder workflow for the current period. Cite the relevant source-system evidence and surface any escalations required.](/tests/security-incident-responder-end-to-end.md)

## Evidence emitted

- sql_result

## Required inputs

- lookup_key
- date_range

## Produces

- incidents_records
- incidents_summary

# Examples

```
query_pagerduty_incidents(lookup_key=<lookup_key>, date_range=<date_range>)
```

# Citations

- [PagerDuty](/systems/pagerduty.md)
