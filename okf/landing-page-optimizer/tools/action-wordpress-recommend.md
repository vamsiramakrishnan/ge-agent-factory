---
type: Agent Tool
title: action_wordpress_recommend
description: Execute the recommend step in WordPress after the agent has gathered evidence and validated escalation gates.
tags:
  - marketing
  - okf
  - brd
timestamp: "2026-07-01T00:00:00.000Z"
source_kind: generationSpec
source_path: behaviorContract.toolIntents
generation_status: generated
ge_status: generated
---

# action_wordpress_recommend

Execute the recommend step in WordPress after the agent has gathered evidence and validated escalation gates.

- **Kind:** action
- **Source system:** [WordPress](/systems/wordpress.md)
- **API:** POST /api/wordpress/recommend

## Inputs

- target_id
- rationale

## Outputs

- action_id
- audit_record_id

## Side Effects

- May change WordPress state because the spec classifies it as action.

## Idempotency

Declared idempotency key: target_id+rationale.

## Confirmation

- [Confirmation policy — action_wordpress_recommend](/policies/confirmation-action-wordpress-recommend.md)

## Permissions

No explicit permission scopes declared; source-system access is tied to [WordPress](/systems/wordpress.md).

## Failure Modes

No explicit failure modes are declared in the spec; rely on refusal/escalation policies for unsafe or incomplete evidence.

## Used By

- [copy_cta_optimization](/workflow/copy-cta-optimization.md)

## Evals

- [Run the Landing Page Optimizer workflow for the current period. Cite the relevant source-system evidence and surface any escalations required.](/tests/landing-page-optimizer-end-to-end.md)

## Evidence emitted

- api_response
- generated_audit_trail

## Required inputs

- target_id
- rationale

## Produces

- action_id
- audit_record_id

# Examples

```
action_wordpress_recommend(target_id=<target_id>, rationale=<rationale>)
```

# Citations

- [WordPress](/systems/wordpress.md)
- [Confirmation policy — action_wordpress_recommend](/policies/confirmation-action-wordpress-recommend.md)
- [Idempotency policy — action_wordpress_recommend](/policies/idempotency-action-wordpress-recommend.md)
