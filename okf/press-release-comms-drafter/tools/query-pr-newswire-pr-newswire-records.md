---
type: Agent Tool
title: query_pr_newswire_pr_newswire_records
description: "Retrieve pr newswire records from PR Newswire for the Press Release & Comms Drafter workflow."
tags:
  - marketing
  - okf
  - brd
timestamp: "2026-07-01T00:00:00.000Z"
source_kind: generationSpec
source_path: behaviorContract.toolIntents
generation_status: generated
ge_status: generated
---

# query_pr_newswire_pr_newswire_records

Retrieve pr newswire records from PR Newswire for the Press Release & Comms Drafter workflow.

- **Kind:** query
- **Source system:** [PR Newswire](/systems/pr-newswire.md)

## Inputs

- lookup_key
- date_range

## Outputs

- pr_newswire_records_records
- pr_newswire_records_summary

## Side Effects

- No mutation is declared by the spec for this tool.

## Idempotency

No idempotency key declared; no mutation is declared by the spec for this tool.

## Confirmation

No write confirmation policy is derivable from the spec.

## Permissions

No explicit permission scopes declared; source-system access is tied to [PR Newswire](/systems/pr-newswire.md).

## Failure Modes

No explicit failure modes are declared in the spec; rely on refusal/escalation policies for unsafe or incomplete evidence.

## Used By

- [approval_distribution](/workflow/approval-distribution.md)

## Evals

- [Run the Press Release & Comms Drafter workflow for the current period. Cite the relevant source-system evidence and surface any escalations required.](/tests/press-release-comms-drafter-end-to-end.md)

## Evidence emitted

- source_system_record

## Required inputs

- lookup_key
- date_range

## Produces

- pr_newswire_records_records
- pr_newswire_records_summary

# Examples

```
query_pr_newswire_pr_newswire_records(lookup_key=<lookup_key>, date_range=<date_range>)
```

# Citations

- [PR Newswire](/systems/pr-newswire.md)
