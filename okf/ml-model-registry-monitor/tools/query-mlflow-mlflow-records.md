---
type: Agent Tool
title: query_mlflow_mlflow_records
description: "Retrieve mlflow records from MLflow for the ML Model Registry & Monitor workflow."
tags:
  - it
  - okf
  - brd
timestamp: "2026-07-01T00:00:00.000Z"
source_kind: generationSpec
source_path: behaviorContract.toolIntents
generation_status: generated
ge_status: generated
---

# query_mlflow_mlflow_records

Retrieve mlflow records from MLflow for the ML Model Registry & Monitor workflow.

- **Kind:** query
- **Source system:** [MLflow](/systems/mlflow.md)

## Inputs

- lookup_key
- date_range

## Outputs

- mlflow_records_records
- mlflow_records_summary

## Side Effects

- No mutation is declared by the spec for this tool.

## Idempotency

No idempotency key declared; no mutation is declared by the spec for this tool.

## Confirmation

No write confirmation policy is derivable from the spec.

## Permissions

No explicit permission scopes declared; source-system access is tied to [MLflow](/systems/mlflow.md).

## Failure Modes

No explicit failure modes are declared in the spec; rely on refusal/escalation policies for unsafe or incomplete evidence.

## Used By

- [performance_metric_collection](/workflow/performance-metric-collection.md)

## Evals

- [Run the ML Model Registry & Monitor workflow for the current period. Cite the relevant source-system evidence and surface any escalations required.](/tests/ml-model-registry-monitor-end-to-end.md)

## Evidence emitted

- source_system_record

## Required inputs

- lookup_key
- date_range

## Produces

- mlflow_records_records
- mlflow_records_summary

# Examples

```
query_mlflow_mlflow_records(lookup_key=<lookup_key>, date_range=<date_range>)
```

# Citations

- [MLflow](/systems/mlflow.md)
