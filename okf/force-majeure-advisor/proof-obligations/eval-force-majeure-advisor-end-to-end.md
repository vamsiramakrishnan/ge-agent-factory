---
type: Proof Obligation
title: "Golden eval obligation — Run the Force Majeure Advisor workflow for the current period. Cite the relevant source-system evidence and surface any escalations required."
description: golden eval proof obligation
source_id: "eval-force-majeure-advisor-end-to-end"
tags:
  - procurement
  - okf
  - brd
timestamp: "2026-07-01T00:00:00.000Z"
source_kind: generationSpec
source_path: behaviorContract.goldenEvals.0
generation_status: generated
ge_status: generated
---

# Golden eval obligation — Run the Force Majeure Advisor workflow for the current period. Cite the relevant source-system evidence and surface any escalations required.

- **Kind:** golden eval
- **Spec source:** behaviorContract.goldenEvals.0
- **Eval:** [force-majeure-advisor-end-to-end](/tests/force-majeure-advisor-end-to-end.md)


## Mechanisms

- [query_contract_repository_contract_repository_records](/tools/query-contract-repository-contract-repository-records.md)
- [query_legal_knowledge_base_legal_knowledge_base_records](/tools/query-legal-knowledge-base-legal-knowledge-base-records.md)
- [query_procurement_3_procurement_3_records](/tools/query-procurement-3-procurement-3-records.md)
- [lookup_force_majeure_advisor_policy_guide](/tools/lookup-force-majeure-advisor-policy-guide.md)
- [action_contract_repository_recommend](/tools/action-contract-repository-recommend.md)

## Entities that must be referenced

- contract_repository_records
- legal_knowledge_base_records
- procurement_3_records

## Forbidden behaviors

- do not invent KPI numbers
- do not skip the evidence_lookup step before any recommendation
- do not execute recommend without two-system evidence

# Citations

- [force-majeure-advisor-policy-guide](/documents/force-majeure-advisor-policy-guide.md)
