---
type: Agent Tool
title: query_sap_fieldglass_gl_entries
description: "Retrieve gl entries from SAP Fieldglass for the Services Procurement & SOW Manager workflow."
tags:
  - procurement
  - okf
  - brd
timestamp: "2026-07-01T00:00:00.000Z"
source_kind: generationSpec
source_path: behaviorContract.toolIntents
generation_status: generated
ge_status: generated
---

# query_sap_fieldglass_gl_entries

Retrieve gl entries from SAP Fieldglass for the Services Procurement & SOW Manager workflow.

- **Kind:** query
- **Source system:** [SAP Fieldglass](/systems/sap-fieldglass.md)

## Inputs

- lookup_key
- date_range

## Outputs

- gl_entries_records
- gl_entries_summary

## Side Effects

- No mutation is declared by the spec for this tool.

## Idempotency

No idempotency key declared; no mutation is declared by the spec for this tool.

## Confirmation

No write confirmation policy is derivable from the spec.

## Permissions

No explicit permission scopes declared; source-system access is tied to [SAP Fieldglass](/systems/sap-fieldglass.md).

## Failure Modes

No explicit failure modes are declared in the spec; rely on refusal/escalation policies for unsafe or incomplete evidence.

## Used By

_Not bound to a workflow stage._

## Evals

- [Run the Services Procurement & SOW Manager workflow for the current period. Cite the relevant source-system evidence and surface any escalations required.](/tests/services-procurement-sow-manager-end-to-end.md)

## Evidence emitted

- source_system_record

## Required inputs

- lookup_key
- date_range

## Produces

- gl_entries_records
- gl_entries_summary

# Examples

```
query_sap_fieldglass_gl_entries(lookup_key=<lookup_key>, date_range=<date_range>)
```

# Citations

- [SAP Fieldglass](/systems/sap-fieldglass.md)
