---
type: Agent Tool
title: query_payscale_payscale_records
description: Retrieve payscale records from Payscale for the Market Benchmarking Analysis Agent workflow.
tags:
  - hr
  - okf
  - brd
timestamp: "2026-07-01T00:00:00.000Z"
source_kind: generationSpec
source_path: behaviorContract.toolIntents
generation_status: generated
ge_status: generated
---

# query_payscale_payscale_records

Retrieve payscale records from Payscale for the Market Benchmarking Analysis Agent workflow.

- **Kind:** query
- **Source system:** [Payscale](/systems/payscale.md)

## Inputs

- lookup_key
- date_range

## Outputs

- payscale_records_records
- payscale_records_summary

## Side Effects

- No mutation is declared by the spec for this tool.

## Idempotency

No idempotency key declared; no mutation is declared by the spec for this tool.

## Confirmation

No write confirmation policy is derivable from the spec.

## Permissions

No explicit permission scopes declared; source-system access is tied to [Payscale](/systems/payscale.md).

## Failure Modes

No explicit failure modes are declared in the spec; rely on refusal/escalation policies for unsafe or incomplete evidence.

## Used By

- [survey_data_aggregation](/workflow/survey-data-aggregation.md)

## Evals

- [Run the Market Benchmarking Analysis Agent workflow for the current period. Cite the relevant source-system evidence and surface any escalations required.](/tests/market-benchmarking-analysis-agent-end-to-end.md)

## Evidence emitted

- source_system_record

## Required inputs

- lookup_key
- date_range

## Produces

- payscale_records_records
- payscale_records_summary

# Examples

```
query_payscale_payscale_records(lookup_key=<lookup_key>, date_range=<date_range>)
```

# Citations

- [Payscale](/systems/payscale.md)
