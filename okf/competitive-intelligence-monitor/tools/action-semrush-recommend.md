---
type: Agent Tool
title: action_semrush_recommend
description: Execute the recommend step in SEMrush after the agent has gathered evidence and validated escalation gates.
tags:
  - marketing
  - okf
  - brd
timestamp: "2026-07-01T00:00:00.000Z"
source_kind: generationSpec
source_path: behaviorContract.toolIntents
generation_status: generated
ge_status: generated
---

# action_semrush_recommend

Execute the recommend step in SEMrush after the agent has gathered evidence and validated escalation gates.

- **Kind:** action
- **Source system:** [SEMrush](/systems/semrush.md)
- **API:** POST /api/semrush/recommend

## Inputs

- target_id
- rationale

## Outputs

- action_id
- audit_record_id

## Side Effects

- May change SEMrush state because the spec classifies it as action.

## Idempotency

Declared idempotency key: target_id+rationale.

## Confirmation

- [Confirmation policy — action_semrush_recommend](/policies/confirmation-action-semrush-recommend.md)

## Permissions

No explicit permission scopes declared; source-system access is tied to [SEMrush](/systems/semrush.md).

## Failure Modes

No explicit failure modes are declared in the spec; rely on refusal/escalation policies for unsafe or incomplete evidence.

## Used By

- [signal_collection](/workflow/signal-collection.md)
- [impact_interpretation](/workflow/impact-interpretation.md)

## Evals

- [Run the Competitive Intelligence Monitor workflow for the current period. Cite the relevant source-system evidence and surface any escalations required.](/tests/competitive-intelligence-monitor-end-to-end.md)

## Evidence emitted

- api_response
- generated_audit_trail

## Required inputs

- target_id
- rationale

## Produces

- action_id
- audit_record_id

# Examples

```
action_semrush_recommend(target_id=<target_id>, rationale=<rationale>)
```

# Citations

- [SEMrush](/systems/semrush.md)
- [Confirmation policy — action_semrush_recommend](/policies/confirmation-action-semrush-recommend.md)
- [Idempotency policy — action_semrush_recommend](/policies/idempotency-action-semrush-recommend.md)
