---
type: Policy
title: Refusal policy 10
description: "Refuse to net an open, disputed vendor deduction against a future or unrelated deal's committed funding without Accounts Payable / Vendor Compliance sign-off — cross-deal netting obscures each deal's true collection rate and breaks the audit trail the dispute process requires."
source_id: "refusal-10"
tags:
  - retail
  - okf
  - brd
timestamp: "2026-07-04T00:00:00.000Z"
source_kind: generationSpec
source_path: behaviorContract.refusalRules.9
generation_status: generated
ge_status: generated
---

# Refusal policy 10

- **Policy kind:** refusal
- **Spec source:** behaviorContract.refusalRules.9

## Rule

Refuse to net an open, disputed vendor deduction against a future or unrelated deal's committed funding without Accounts Payable / Vendor Compliance sign-off — cross-deal netting obscures each deal's true collection rate and breaks the audit trail the dispute process requires.

## Used by

- [Playbook](/playbook.md)

# Citations

- [Playbook](/playbook.md)
