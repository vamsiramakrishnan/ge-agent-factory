---
type: Agent Tool
title: query_survey_platform_survey_platform_records
description: Retrieve survey platform records from Survey Platform for the Onboarding Effectiveness Analyzer workflow.
tags:
  - hr
  - okf
  - brd
timestamp: "2026-07-01T00:00:00.000Z"
source_kind: generationSpec
source_path: behaviorContract.toolIntents
generation_status: generated
ge_status: generated
---

# query_survey_platform_survey_platform_records

Retrieve survey platform records from Survey Platform for the Onboarding Effectiveness Analyzer workflow.

- **Kind:** query
- **Source system:** [Survey Platform](/systems/survey-platform.md)

## Inputs

- lookup_key
- date_range

## Outputs

- survey_platform_records_records
- survey_platform_records_summary

## Side Effects

- No mutation is declared by the spec for this tool.

## Idempotency

No idempotency key declared; no mutation is declared by the spec for this tool.

## Confirmation

No write confirmation policy is derivable from the spec.

## Permissions

No explicit permission scopes declared; source-system access is tied to [Survey Platform](/systems/survey-platform.md).

## Failure Modes

No explicit failure modes are declared in the spec; rely on refusal/escalation policies for unsafe or incomplete evidence.

## Used By

- [signal_aggregation](/workflow/signal-aggregation.md)

## Evals

- [Run the Onboarding Effectiveness Analyzer workflow for the current period. Cite the relevant source-system evidence and surface any escalations required.](/tests/onboarding-effectiveness-analyzer-end-to-end.md)

## Evidence emitted

- source_system_record

## Required inputs

- lookup_key
- date_range

## Produces

- survey_platform_records_records
- survey_platform_records_summary

# Examples

```
query_survey_platform_survey_platform_records(lookup_key=<lookup_key>, date_range=<date_range>)
```

# Citations

- [Survey Platform](/systems/survey-platform.md)
