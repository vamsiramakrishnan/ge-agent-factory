---
type: Agent Tool
title: query_apigee_apigee_records
description: Retrieve apigee records from Apigee for the Integration Pattern Advisor workflow.
tags:
  - it
  - okf
  - brd
timestamp: "2026-07-01T00:00:00.000Z"
source_kind: generationSpec
source_path: behaviorContract.toolIntents
generation_status: generated
ge_status: generated
---

# query_apigee_apigee_records

Retrieve apigee records from Apigee for the Integration Pattern Advisor workflow.

- **Kind:** query
- **Source system:** [Apigee](/systems/apigee.md)

## Inputs

- lookup_key
- date_range

## Outputs

- apigee_records_records
- apigee_records_summary

## Side Effects

- No mutation is declared by the spec for this tool.

## Idempotency

No idempotency key declared; no mutation is declared by the spec for this tool.

## Confirmation

No write confirmation policy is derivable from the spec.

## Permissions

No explicit permission scopes declared; source-system access is tied to [Apigee](/systems/apigee.md).

## Failure Modes

No explicit failure modes are declared in the spec; rely on refusal/escalation policies for unsafe or incomplete evidence.

## Used By

- [pattern_retrieval](/workflow/pattern-retrieval.md)

## Evals

- [Run the Integration Pattern Advisor workflow for the current period. Cite the relevant source-system evidence and surface any escalations required.](/tests/integration-pattern-advisor-end-to-end.md)

## Evidence emitted

- source_system_record

## Required inputs

- lookup_key
- date_range

## Produces

- apigee_records_records
- apigee_records_summary

# Examples

```
query_apigee_apigee_records(lookup_key=<lookup_key>, date_range=<date_range>)
```

# Citations

- [Apigee](/systems/apigee.md)
