...
| `GET /api/ge/evals/coverage` | `ge evals coverage` | Report per-dimension coverage (required/covered/gaps) from the last `ge evals compile`, optionally scoped to one evalset id | `read-only` | `bun` on PATH |
| `POST /api/ge/init` | `ge init` | Discover project/config values from Terraform outputs and gcloud, then write .ge.json | `writes-repo` | `gcloud` on PATH · writable `.ge.json` |
