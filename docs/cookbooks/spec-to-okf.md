...
> Path note: the OKF scripts live under **`apps/ge-demo-generator/scripts/`**,
...
1. **Convert a spec to an OKF bundle — by use-case id:**
   ...
   ```bash
   node apps/ge-demo-generator/scripts/spec-to-okf.mjs --id <useCaseId>
   ```

   **or from a spec file:**

   ```bash
   node apps/ge-demo-generator/scripts/spec-to-okf.mjs --spec <path.json>
   ```

   Flags (exactly four): `--id <useCaseId>` *or* `--spec <path.json>` (one
   required, mutually exclusive), `--out <dir>` (default
   `apps/ge-demo-generator/artifacts/okf/<id>/`), `--help`. The command prints
...
node apps/ge-demo-generator/scripts/spec-to-okf.mjs --id <useCaseId>   # conceptCount > 0
ls apps/ge-demo-generator/artifacts/okf/<useCaseId>/                   # index.md, playbook.md, systems/, ...

# round-trip is parseable:
node apps/ge-demo-generator/scripts/okf-to-spec.mjs --bundle apps/ge-demo-generator/artifacts/okf/<useCaseId>/
