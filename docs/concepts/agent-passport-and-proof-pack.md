...
| `artifacts/generator-feedback.json` | an independent harness review scored the code against the contract |
| `artifacts/harness-refine.json` | what was auto-fixed, with a `spec_to_code_fidelity` verdict |
...
| Agent Registry entry (`register_tools` stage; modes `adk` / `mcp` / `a2a`) | the agent and the toolsets it may resolve |
...
- **Console:** the Agent detail view (stage pipeline, artifacts, live
  logs); the **Runs** view for history; **Fleet** for the same rolled up
  across every agent.
