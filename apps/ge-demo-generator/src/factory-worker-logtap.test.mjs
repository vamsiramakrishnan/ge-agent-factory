...
import { makeLedgerLogTap, resolveBuilderServiceAccount, resolveBuildLogsDir } from "./factory-worker.js";
...
test("resolveBuildLogsDir: default derives gs://<artifactBucket>/cloudbuild-logs", () => {
  expect(resolveBuildLogsDir({ cloud: { artifactBucket: "proj-ge-agent-factory" } }, {})).toBe(
    "gs://proj-ge-agent-factory/cloudbuild-logs",
  );
});

test("resolveBuildLogsDir: falls back to GE_AGENT_FACTORY_BUCKET env", () => {
  expect(resolveBuildLogsDir({ cloud: {} }, { GE_AGENT_FACTORY_BUCKET: "env-bucket" })).toBe(
    "gs://env-bucket/cloudbuild-logs",
  );
});

test("resolveBuildLogsDir: explicit GE_AGENT_FACTORY_BUILD_LOGS_DIR override wins", () => {
  expect(
    resolveBuildLogsDir({ cloud: { artifactBucket: "ignored" } }, { GE_AGENT_FACTORY_BUILD_LOGS_DIR: "gs://other/logs" }),
  ).toBe("gs://other/logs");
});

test("resolveBuildLogsDir: empty when no bucket available", () => {
  expect(resolveBuildLogsDir({ cloud: {} }, {})).toBe("");
});
