...
import { nextEventSeq } from "@ge/run-ledger/frames";
...
export function resolveBuildLogsDir(payload = {}, env = process.env) {
  const override = env.GE_AGENT_FACTORY_BUILD_LOGS_DIR;
  if (override) return override;
  const bucket = payload.cloud?.artifactBucket || env.GE_AGENT_FACTORY_BUCKET || "";
  if (!bucket) return "";
  const normalized = bucket.startsWith("gs://") ? bucket.replace(/\/+$/, "") : `gs://${bucket.replace(/\/+$/, "")}`;
  return `${normalized}/cloudbuild-logs`;
}
...
export function buildStageExecutionPlan(payload) {
  ...
  const planDataArgs = ["scripts/ge-mock.mjs", "plan-data", "--dir", workspaceDir];
  ...
  if (RELEASE_STAGES.has(payload.stage)) {
    ...
    // logs bucket), so we also pass --gcs-log-dir pointing at the factory bucket.
    // Both are env-overridable for testability; defaults reconstruct the conventional
    // builder SA email + the factory artifact bucket the worker already uses.
    const buildServiceAccount = resolveBuilderServiceAccount(project, process.env);
    const buildLogsDir = resolveBuildLogsDir(payload, process.env);
    const buildArgs = [
      "builds", "submit",
      "--no-source",
    ...
    if (buildLogsDir) buildArgs.push("--gcs-log-dir", buildLogsDir);
  ...
  const commandsByStage = {
    generate_data: [["node", ["scripts/ge-mock.mjs", "generate", "--dir", workspaceDir]]],
    package_data: [
      ["node", planDataArgs],
      ["node", ["scripts/ge-mock.mjs", "snowfakery-recipe", "--dir", workspaceDir]],
      ["node", ["scripts/ge-mock.mjs", "data-plan", "--dir", workspaceDir, "--project", project, "--location", region]],
    ],
    ...
    harness_refine: payload.options?.refine === false ? [] : (() => {
      ...
      return [
        ...
        ["node", ["scripts/ge-mock.mjs", "harness-review", "--dir", workspaceDir, "--provider", provider, "--vertex", "true", "--project", project, "--location", refineLocation, "--model", harnessModel, "--soft", "true"]],
    ...
    verify_live: [["node", ["scripts/ge-mock.mjs", "status", "--dir", workspaceDir]]],
  };
...
export async function runFactoryWorker(payload, { dryRun = false } = {}) {
  ...
  if (restore.ok === false) {
    const failed = { status: "failed", stage: payload.stage, owner: "cloud_run_service", outputs: [], nextStage: null, error: restore.error };
    await recordStageEvent(payload, { type: "stage_failed", status: "failed", owner: failed.owner, error: failed.error });
    await sink.close();
    return failed;
  }
  ...
  const commandEnv = {
    ...
    // an operator-set output-token budget so `ge-mock generate` matches local builds.
  ...
  for (const [cmd, args] of plan.commands) {
    ...
    if (result.code !== 0) {
      const failed = { status: "failed", stage: payload.stage, owner: plan.owner, outputs, nextStage: null, error: `${cmd} exited ${result.code}` };
      await logTap.stop();
  ...
  if (RELEASE_STAGES.has(payload.stage)) {
      ...
      if (!CLOUD_BUILD_TERMINAL_SUCCESS.has(build.status)) {
        if (CLOUD_BUILD_TERMINAL_FAILURE.has(build.status)) {
          const failed = {
            ...
            outputs,
            nextStage: null,
            error: `Cloud Build ${build.status}`,
            logUrl: build.logUrl,
          };
          sink.log(makeEvent({ ...meta, type: "stage_failed", level: "error", data: { owner: plan.owner, error: failed.error } }));
    ...
    if (!cloudBuildId) {
      const failed = { status: "failed", stage: payload.stage, owner: plan.owner, outputs, nextStage: null, error: "Cloud Build submitted but build id was not found" };
      sink.log(makeEvent({ ...meta, type: "stage_failed", level: "error", data: { owner: plan.owner, error: failed.error } }));
      await recordStageEvent(payload, { type: "stage_failed", status: "failed", owner: plan.owner, error: failed.error });
      await persistArtifacts(payload, failed);
      await sink.close();
