...
function resolveCloudTopology(options = {}) {
  ...
  const namespace = safeCloudName(`${prefix}-${environment}`, "ge-mock", 32);
  ...
  return {
    ...
    services: {
      alloydb: {
        ...
        cluster: safeCloudName(`${namespace}-alloydb`, "ge-mock-alloydb", 40),
        instance: safeCloudName(`${namespace}-primary`, "ge-mock-primary", 40),
      ...
      bigtable: {
        ...
        instance: safeCloudName(`${namespace}-events`, "ge-mock-events", 30),
      ...
      gcs: {
        ...
        bucket: options.bucket || `${projectToken}-ge-mock-data`,
        prefixPattern: "ge-mock/<environment>/<workspace_id>",
...
function workspaceCloudTargets(item, topology) {
  ...
  return {
    ...
    gcs: {
      ...
      prefix: `ge-mock/${topology.environment}/${workspaceToken}`,
    },
  };
}
...
function runGeMock(repoRoot, args, { verbose = false } = {}) {
  const result = spawnSync(process.execPath, ["scripts/ge-mock.mjs", ...args], {
    cwd: repoRoot,
    env: process.env,
    encoding: "utf8",
    maxBuffer: 30 * 1024 * 1024,
  ...
  if (verbose && stderr) process.stderr.write(stderr);
  if (result.error || result.status !== 0) {
    throw new Error(`node scripts/ge-mock.mjs ${args.join(" ")} failed: ${result.error?.message || stderr || stdout}`);
  }
  return parseJsonObjects(stdout);
}
