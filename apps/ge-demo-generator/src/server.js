...
async function startAgentRun(payload, res = null, req = null) {
  ...
  if (agentRecord) {
    promptParts.push(
      ...
      "Workspace-local commands are on PATH: `ge validate`, `ge status`, `ge-mock status`. Do not change directory to the repository root from this sandboxed run.",
    );
  } else {
    promptParts.push("", "Write files for this task inside the workspace unless the user explicitly asks to change the harness repo itself.", "Workspace-local commands are on PATH: `ge validate`, `ge status`, `ge-mock status`.");
  }
