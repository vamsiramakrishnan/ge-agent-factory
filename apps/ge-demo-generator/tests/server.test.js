...
test("factory worker parses payloads and dispatches release stages to Cloud Build", async () => {
  ...
  // cloudbuild.builds.editor) with an explicit logs destination (required for a
  // user-specified build SA).
  ...
  assert.ok(deployArgsStr.includes("gs://vital-octagon-19612-ge-agent-factory/cloudbuild-logs"));
