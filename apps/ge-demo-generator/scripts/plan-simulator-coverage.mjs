...
import { findSimulatorForSystem, loadSimulatorRegistry } from "./ge-mock/simulators/registry.mjs";
...
async function main() {
  ...
  const sourcePath = resolve(flags.source || "apps/ge-demo-generator/src/use-case-source-map.generated.json");
  const inventoryPath = resolve(flags.inventory || "apps/ge-demo-generator/artifacts/upstream-system-inventory.json");
  const outJson = resolve(flags.json || "apps/ge-demo-generator/artifacts/simulator-coverage.json");
  const outMd = resolve(flags.md || "apps/ge-demo-generator/artifacts/simulator-coverage.md");
