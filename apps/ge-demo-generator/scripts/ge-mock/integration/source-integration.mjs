...
export async function buildSourceIntegrationPlan(dir, flags = {}, opts = {}) {
    ...
    return {
      ...
      mcpStrategy: {
        ...
        customMcpAdapter: customMcpNeeded ? {
          ...
          registerCommand: `ge-mock register --dir ${dir} --as mcp --project ${project} --region ${location}`,
          adkUsage: [
            "from google.adk.auth.credential_manager import CredentialManager",
            "from google.adk.integrations.agent_identity import GcpAuthProvider",
            "from google.adk.integrations.agent_registry import AgentRegistry",
  ...
  const plan = {
    ...
    googleCloud: {
      ...
      customAdapterDeployCommand: apiContract ? `bash mock_data/apis/deploy-adapter.sh && ge-mock register --dir ${dir} --as mcp --project ${project} --region ${location}` : null,
    },
    registries: {
      toolRegistry: {
      ...
      agentRegistry: {
        adkRuntime: `ge-mock register --dir ${dir} --as adk --project ${project} --region ${location}`,
        mcpServers: customMcpSources.length ? `ge-mock register --dir ${dir} --as mcp --project ${project} --region ${location}` : null,
      },
    },
    artifacts: {
