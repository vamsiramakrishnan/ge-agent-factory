...
// The query-tool name a table materializes to. This MUST mirror tableToolName in
// ge-mock.mjs (source-prefix collapse) so dedup happens at the SAME key the Python
// emitter uses. Two tables collapsing to one tool (e.g. owned "gl_entries" on
// source sap_s_4hana_fi and derived "sap_s_4hana_fi_gl_entries") would otherwise
// emit `query_sap_s_4hana_fi_gl_entries` twice — the 2nd stub shadowing the real one.
function tableToolKey(sourceSystemId, name) {
  const source = snakeCase(sourceSystemId || "source");
  const tableName = snakeCase(name || "records");
  const deduped = tableName === source || tableName.startsWith(`${source}_`)
    ? tableName.slice(source.length).replace(/^_+/, "") || "records"
    : tableName;
  return snakeCase(`${source}_${deduped}`);
}
...
export function ensureContractQueryTables(schema) {
  ...
  // Dedup at the emitted TOOL name, not the table name. An owned table and a
  // contract-derived table can have different names yet collapse to the same
  // query_* tool (prefix-collapse); deriving the second would shadow the real one.
  const tableSourceId = (table) => table._sourceSystemId || table.sourceSystemId || "";
  const existingToolKeys = new Set(
    (schema.tables || []).map((table) => tableToolKey(tableSourceId(table), table.name)),
  );
  const defaultRows = schema.rowPolicy?.defaultRowsPerEntity || schema.useCaseSpec?.rowPolicy?.defaultRowsPerEntity || 25;
  const additions = [];
  for (const intent of intents) {
    // Skip if the tool this intent materializes already exists (real or owned),
    // and also dedup intents among themselves by tool key.
    const toolKey = tableToolKey(intent.sourceSystemId, materializedTableName(intent));
    if (existingToolKeys.has(toolKey)) continue;
