...
import { buildScenarioGraph } from "./ge-mock/scenario-graph.mjs";
import { buildSimulatorProjections } from "./ge-mock/projections/simulator-projections.mjs";
import { matchScenarioPacks } from "./ge-mock/packs/index.mjs";
...
function lookupCandidates(useCase = {}) {
  return [
    useCase.id,
    useCase.title,
    useCase.path,
...
function scoreUseCaseMatch(useCase, requested) {
...
function resolveUseCase(sourceMap, requested) {
...
function useCaseNotFoundMessage(usecase, suggestions = [], ambiguous = false) {
...
function sourceKindForSystem(system = {}) {
...
function useCaseFromSpec(spec = {}, sourcePath = null) {
...
function snowExpression(expression) {
  return `\${{${expression}}}`;
}
...
function lakehouseClass(source) {
...
function referenceTargetFor(column, { availableObjects = new Set(), currentObject = null } = {}) {
  const candidates = {
...
function buildPackBridgeContext(useCase) {
...
function packBridgesForUseCase(useCase) {
...
function attachPackBridgesToUseCase(useCase, packBridges) {
  ...
  for (const datastore of plan.datastores) {
    ...
    if (datastore.id === "gcs") {
      ...
      await writeText(join(workspace, "mock_data", "blobs", "load-gcs.sh"), [
        ...
        "gcloud storage cp --recursive mock_data/blobs/gcs/objects \"gs://${GCS_BUCKET}/${GCS_PREFIX:-ge-mock}/documents\"",
        "",
      ].join("\n"));
    }
  ...
  const plan = {
    ...
    dataRealizers: [
      ...
      },
      {
        ...
        output: "mock_data/simulators/",
        runCommand: "node apps/ge-demo-generator/scripts/materialize-simulator-seeds.mjs --dir <workspace>",
