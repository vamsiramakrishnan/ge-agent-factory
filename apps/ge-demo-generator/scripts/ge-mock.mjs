#!/usr/bin/env node
/**
 * ge-mock — Unified mock system pipeline CLI
 *
 ...
 *
 * USAGE:
 *   ge-mock init     --name <name> --domain <domain> --dir <workspace-dir>
 *   ge-mock schema   --dir <dir> --add-table <json>
 *   ge-mock generate --dir <dir> [--seed N] [--rows N]
 *   ge-mock tools    --dir <dir>
 *   ge-mock test     --dir <dir>
 *   ge-mock serve    --dir <dir> [--port N]
 *   ge-mock deploy   --dir <dir> --project <gcp-project> --region <region>
 *   ge-mock register --dir <dir> [--as mcp|a2a]
 *   ge-mock publish  --dir <dir> --app-id <gemini-enterprise-app-id>
 *   ge-mock status   --dir <dir>
 *   ge-mock reset    --dir <dir> --step <step>
 */
...
import { canonicalSystemId, safePyName, snakeCase, titleCase, validPythonIdentifierName } from "./ge-mock/core/naming.mjs";
import { CONTRACT_INTENT_KINDS, CONTRACT_TOOL_KINDS, ensureContractQueryTables, inferEvalToolArgs, tablePrimaryKey } from "./ge-mock/core/contract-schema.mjs";
import { MANAGED_MCP_SERVICES, buildSourceIntegrationPlan } from "./ge-mock/integration/source-integration.mjs";
import { renderToolsModule } from "./ge-mock/runtime/tools-backend.mjs";
import { findSimulatorForSystem, loadSimulatorRegistry, simulatorBindingForTool } from "./ge-mock/simulators/registry.mjs";
import { matchPipelineSteps } from "./ge-mock/agent-workflow.mjs";
...
import { buildSemanticModel } from "./ge-mock/data/semantic-model.mjs";
...
function requireStep(pipeline, step) {
  if (!pipeline.steps[step] || pipeline.steps[step].status !== "done") {
    console.error(`Step "${step}" has not been completed yet. Run "ge-mock ${step}" first.`);
    process.exit(1);
  }
}
...
async function cmdGenerate(dir, flags) {
  ...
  if (!schema || !schema.tables?.length) fail("No schema found. Run 'ge-mock schema --add-table ...' first.");
...
async function ensureAgentsCliPyprojectMetadata(dir, manifest, { deploymentTarget = "none" } = {}) {
  ...
  if (!text.trim()) {
    text = [
      `[project]`,
      `name = "${snakeCase(manifest?.id || "generated_agent")}"`,
      `version = "0.1.0"`,
      `requires-python = ">=3.11"`,
      `dependencies = ["google-adk", "pydantic>=2", "pytest>=8"]`,
      ``,
  ...
  const projectName = snakeCase(manifest?.id || "generated_agent");
...
async function cmdTools(dir, flags) {
  ...
  if (!manifest) fail("No manifest found. Run 'ge-mock generate' first.");
...
async function cmdTest(dir, flags) {
  ...
  const manifest = await readJson(manifestPath(dir), null);
  if (!manifest) fail("No manifest. Run 'ge-mock generate' first.");
...
async function cmdQualityGate(dir, flags) {
  ...
  try {
    ...
    if (runHarness) {
      ...
      results.push({ name: "harness-review", command: ["ge-mock", "harness-review"], exitCode: 0, ok: true, stdout: JSON.stringify(harnessReview), stderr: "" });
    }
    let harnessRefine = null;
    if (runRefine) {
      harnessRefine = await cmdHarnessRefine(dir, flags);
      results.push({ name: "harness-refine", command: ["ge-mock", "harness-refine"], exitCode: 0, ok: true, stdout: JSON.stringify(harnessRefine), stderr: "" });
    }
...
async function cmdHarnessReview(dir, flags) {
  ...
  if (!manifest && !spec) fail("No generated workspace context found. Run 'ge-mock from-usecase' or 'ge-mock tools' first.");
  await mkdir(join(dir, "artifacts"), { recursive: true }).catch(() => {});
...
function bigQueryType(col) {
  const type = String(col?.type || "string").toLowerCase();
  if (type === "number") return "INT64";
...
async function buildCloudDataArtifacts(dir, flags = {}) {
  ...
  requireStep(pipeline, "generate");
  const manifest = await readJson(manifestPath(dir), null);
  if (!manifest) fail("No fixture manifest. Run 'ge-mock generate' first.");
  ...
  const bucket = flags.bucket || `${project}-ge-mock-${scenario}`.toLowerCase().replace(/[^a-z0-9._-]/g, "-");
  const prefix = (flags.prefix || `ge-mock/${scenario}`).replace(/^\/+|\/+$/g, "");
  ...
  await writeJson(join(outDir, "mcp-tools.json"), {
    ...
    simulatorRegistry: {
      path: "apps/ge-demo-generator/simulator-systems/registry.json",
  ...
  await writeJson(deployPlanPath(dir), {
    ...
    nextCommands: [
      ...
      `ge-mock deploy --dir ${dir} --project ${project} --region ${flags.region || "us-central1"} --target agent_runtime`,
    ],
  });
...
async function cmdSnowfakeryRecipe(dir, flags) {
  ...
  if (!schema?.tables?.length) fail("No schema tables found. Run 'ge-mock schema' first.");
  ...
  const manifest = {
    ...
    note: "ge-mock writes the recipe and packager plan. Install/run Snowfakery only when materializing rows through the external Snowfakery engine.",
  };
  ...
  try {
    ...
    if (noWait && !runtimeId) {
      markStep(pipeline, "deploy", "running", {
        ...
        target,
        operation,
        statusCommand: `ge-mock deploy-status --dir ${dir} --project ${project} --region ${region}`,
      });
      await savePipeline(dir, pipeline);
      ok({
        step: "deploy",
...
async function cmdRegister(dir, flags) {
  ...
  ok({
    ...
    mode: "adk",
    runtimeId,
    nextStep: `ge-mock publish --dir ${dir} --app-id <GEMINI_ENTERPRISE_APP_ID>`,
  });
}
...
async function cmdPublish(dir, flags) {
  ...
  if (regType === "adk" && !runtimeId) {
    fail("No valid Agent Runtime ID found. Run ge-mock deploy-status until deployment writes deployment_metadata.json before publishing.");
  }
...
async function cmdStatus(dir) {
  ...
  ok({
    ...
    nextStep,
    nextCommand: nextStep !== "done" ? `ge-mock ${nextStep} --dir ${dir}` : null,
    schema: schema ? {
...
async function cmdFromUseCase(dir, flags) {
    ...
    try {
      ...
      if (!useCase) {
        ...
        if (matches.length === 1) useCase = matches[0];
          ...
          return;
        } else fail(`Use case "${useCaseId}" not found. Use ge-mock list-usecases to browse.`);
      }
    } catch (e) {
  ...
  ok({
    ...
    nextSteps: [
      reviewResult.skipped ? `ge-mock harness-review --dir ${targetDir} --vertex true --project <project> --location <location>` : null,
      refineResult.skipped ? `ge-mock harness-refine --dir ${targetDir} --vertex true --project <project> --location <location>` : null,
      `ge-mock serve --dir ${targetDir}`,
      `ge-mock status --dir ${targetDir}`,
    ].filter(Boolean),
  });
}
...
async function cmdBatchAudit(flags) {
  ...
  const root = resolve(flags.root || `/tmp/ge-mock-batch-audit-${timestamp}`);
...
function printHelp() {
  console.log(`
ge-mock — Unified mock system pipeline
...
Examples:
  ...
  ge-mock schema --dir ./hr-agent --add-table '{"name":"employees","rows":50,"columns":[{"name":"id","type":"seq","pattern":"EMP-{n:4}"},{"name":"name","type":"person.fullName"},{"name":"dept","type":"enum","values":["HR","IT","Finance"]}]}'
  ge-mock generate --dir ./hr-agent --seed 42
  ge-mock tools --dir ./hr-agent
  ge-mock test --dir ./hr-agent
  ge-mock eval --dir ./hr-agent --run false
  ge-mock quality-gate --dir ./hr-agent --prompt "hello" [--evals true|false] [--harness-review true --vertex true|false --no-vertex --project my-project --location global]
  ge-mock harness-review --dir ./hr-agent [--provider antigravity-sdk|agy|gemini|codex|claude] [--vertex true|false --no-vertex --project my-project --location global]
  ge-mock harness-refine --dir ./hr-agent [--provider antigravity-sdk] [--vertex true|false --no-vertex --project my-project --location global]
  ge-mock pack-coverage [--out artifacts/pack-coverage.json]
  ge-mock batch-audit [--limit 25] [--department hr] [--root /tmp/ge-audit] [--run true|false] [--harness-review true]
  ge-mock sources --slides ../presentation/src/components/slides/use-cases
  ge-mock plan-data --dir ./hr-agent --usecase BenefitsAssistant
  ge-mock data-plan --dir ./hr-agent --project my-gcp-project --location US
  ge-mock source-integration-plan --dir ./hr-agent --project my-gcp-project --location global
  ge-mock snowfakery-recipe --dir ./hr-agent
  ge-mock mcp --action enable --service bigquery --project my-gcp-project
  ge-mock deploy --dir ./hr-agent --project my-gcp-project --target agent_runtime
  ge-mock deploy-status --dir ./hr-agent --project my-gcp-project --region us-central1
  ge-mock verify-live --dir ./hr-agent --prompt "hello"
  ge-mock deploy --dir ./hr-agent --project my-gcp-project --target cloud_run
  ge-mock register --dir ./hr-agent --as mcp
  ge-mock publish --dir ./hr-agent --app-id my-gemini-enterprise-app-id
  ge-mock status --dir ./hr-agent
`);
}
