...
resources:
  scripts:
    - path: scripts/check-api-package.mjs
      ...
      use_when: Auditing a workspace after ge-mock plan-data.
  references:
...
`ge-mock plan-data` owns generation. It should emit API artifacts under `mock_data/apis/` whenever the use-case source map includes service protocols.
