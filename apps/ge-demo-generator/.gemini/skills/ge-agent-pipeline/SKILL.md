...
Prefer `ge` commands. Use `ge-mock` only for targeted regeneration inside an existing workspace.
...
If the workspace is missing several of these, run `ge create` or use `ge-mock from-usecase` only as a fallback.
...
## When Direct `ge-mock` Is Acceptable

Use direct `ge-mock` only for a narrow repair:
...
ge-mock generate --dir <workspace> --seed 42
ge-mock tools --dir <workspace>
ge-mock test --dir <workspace>
