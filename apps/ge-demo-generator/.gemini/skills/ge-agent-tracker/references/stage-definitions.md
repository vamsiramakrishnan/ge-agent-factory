...
- Workspace directory initialized (`ge-mock init`)
- `mock_systems/pipeline.json` exists with `init` step marked `done`
- `mock_systems/schema.json` exists (may be empty)

**Exit criteria**:
- Schema has at least one table defined via `ge-mock schema --add-table`
- OR: `ge-mock from-usecase` was used (auto-derives schema)
...
**CLI command**: `ge-mock init --dir <dir> --name <name> --domain <domain>`
...
- `ge-mock generate` succeeded: fixture files exist in `fixtures/tables/`
- `ge-mock tools` succeeded: `app/tools.py`, `app/agent.py`, `app/__init__.py` exist
...
**Shortcut**: `ge-mock from-usecase` runs init + schema + generate + tools in
...
- `ge-mock test` generated `tests/test_smoke.py`
...
**CLI command**: `ge-mock test --dir <dir>`
...
- `ge-mock serve` starts without error
...
**CLI command**: `ge-mock serve --dir <dir> [--port 8080]`
...
**CLI command**:
```bash
ge-mock deploy --dir <dir> --project <p> --target agent_runtime
ge-mock deploy --dir <dir> --project <p> --target cloud_run
```
...
**CLI command**:
```bash
ge-mock register --dir <dir> --as mcp   # MCP server
ge-mock register --dir <dir> --as a2a   # A2A remote agent
ge-mock register --dir <dir> --as adk   # Ready for Gemini Enterprise
```
