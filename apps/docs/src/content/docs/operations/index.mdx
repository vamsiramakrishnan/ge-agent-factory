---
title: "Operations"
editUrl: "https://github.com/vamsiramakrishnan/ge-agent-factory/edit/main/docs/OPERATIONS.md"
sidebar:
  label: Overview
  order: 0
---
...
|---|---|---|
| Gateway | `ge-agent-factory-gateway` | Scaffolds each agent <a href="/ge-agent-factory/reference/glossary/#workspace" title="The on-disk folder for one generated agent (.ge/factory/workspaces/workspace-id/), with a workspace.json manifest describing what's in it.">workspace</a> in-process; enqueues the <a href="/ge-agent-factory/reference/glossary/#pipeline" title="The user-facing, end-to-end path from business intent to a deployed agent — interview → spec → data → simulator → build → eval → preview → deploy — rendered as a plan you can read (ge pipeline plan) and watch (ge pipeline status).">pipeline</a>. Browser/UI entry (IAP-protected). |
...
<p align="center">
  <img src="/ge-agent-factory/assets/diagrams/factory-line.svg" alt="plan, generate_workspace, generate_data, package_data, harness_refine, validate, preview, plan_deploy, load_data, deploy_runtime, poll_runtime, register_tools, publish_enterprise, verify_live" width="700" />
</p>
...
`harness_refine` runs **<a href="/ge-agent-factory/reference/glossary/#antigravity" title="Google's agent SDK (the google-antigravity Python package) that powers the harness review/refine step — the factory drives it headlessly to judge and repair spec-to-code fidelity.">Antigravity</a> review+refine** (with the best ADK skills) on
...
`--soft`, so a Vertex outage or <a href="/ge-agent-factory/reference/glossary/#harness" title="The local, LLM-driven &quot;does this generated agent actually match the spec?&quot;">harness</a> error degrades to the deterministic code
...
Run bare `ge` anytime for a status board (mode · <a href="/ge-agent-factory/reference/glossary/#planes" title="The three infrastructure layers the platform stands up in your cloud project — the factory plane (gateway, worker, Cloud Tasks queue, Cloud Build: the machinery that runs builds), the data plane (GCS, BigQuery, Bigtable, Firestore, AlloyDB, per-agent data + IAM), and the tool plane (the per-department MCP services, Agent Registry entries, and the Agent Gateway that agents call tools through).">planes</a> ✓/○ · next command).
...
the factory. Run these once per project, before/with the <a href="/ge-agent-factory/reference/glossary/#fleet" title="The whole set of generated agents/workspaces, treated as one unit — the opposite end of the scale from a canary.">fleet</a> provision:
...
There is a hard **<a href="/ge-agent-factory/reference/glossary/#build-boundary" title="The cutoff point between &quot;building and previewing an agent on your own machine&quot; and &quot;actually deploying it to your Google Cloud project.&quot;">build boundary</a>**: everything up to validate/preview is pure
...
without re-generating: the <a href="/ge-agent-factory/reference/glossary/#handoff" title="The factory's release act: giving a locally proven workspace to the layer below — agents-cli → ADK Agent Engine → Gemini Enterprise — via ge handoff agents-cli, which runs only the post-boundary stages (load_data → deploy_runtime → register_tools → publish_enterprise).">handoff</a> tars + uploads each prebuilt workspace and
...
| Path | Purpose |
|---|---|
| `.ge/runtime/` | <a href="/ge-agent-factory/reference/glossary/#daemon" title="The local background process (ge daemon start, HTTP on port 17654 / GE_DAEMON_PORT) that runs long factory work — pipeline runs, repair runs, generic jobs — as durable runtime tasks, so a run survives your terminal closing and can be resumed.">daemon</a> task records, events, interaction responses, resume plans |
| `.ge/pipelines/` | pipeline graph workspaces, mock data, Snowfakery output, simulator overlays |
| `.ge/interviews/` | generated <a href="/ge-agent-factory/reference/glossary/#interview" title="The guided, conversational flow where a business user (or agent) describes a use case and the factory turns it into a formal spec.">interview</a> specs before/after registry review |
...
<p align="center">
  <img src="/ge-agent-factory/assets/diagrams/access-posture-tiers.svg" alt="Gateway and worker use IAM/OIDC authenticated-only ingress with no IAP, while console and presentation use direct Cloud Run IAP restricted to iap_members" width="700" />
</p>
...
The console's views are documented in [Console](/ge-agent-factory/console/); operationally:
...
- [Troubleshooting](/ge-agent-factory/operations/troubleshooting/) — failure modes we've hit, with fixes.
- [Provision the platform](/ge-agent-factory/operations/provision-the-platform/) — standing the planes up.
- [Deploy the Agent Gateway](/ge-agent-factory/operations/agent-gateway/) — the governed tool front door.
- [Run and observe](/ge-agent-factory/operations/run-and-observe/) — runs, resume, and the durable record.
- Archived: [live rename cutover runbook](https://github.com/vamsiramakrishnan/ge-agent-factory/blob/main/docs/runbooks/rename-cutover.md).
