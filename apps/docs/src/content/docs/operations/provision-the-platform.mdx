...
Stand up the cloud <a href="/ge-agent-factory/reference/glossary/#planes" title="The three infrastructure layers the platform stands up in your cloud project: the factory plane, the data plane, and the tool plane.">planes</a> the remote factory needs — the factory/runtime plane,
the data plane, and the tool (Model Context Protocol, MCP) plane — and drive
them to a green readiness verdict with the <a href="/ge-agent-factory/reference/glossary/#doctor" title="The health-check family: a set of read-only, actionable pass/fail checks over the toolchain and the three planes.">doctor</a>. Or skip the plane-by-plane
