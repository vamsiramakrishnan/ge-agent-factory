 ...
 * tags. Ported from the pixelpitch reference (apps/web/src/artifacts/parser.ts):
 * handles one artifact at a time, ignores nesting.
 *
 * Feed deltas in, iterate events. The interview canvas uses this to render the
 * agent-spec as it streams inside the antigravity.text_delta channel, before the
 * agent-spec.json file lands on disk.
 */
