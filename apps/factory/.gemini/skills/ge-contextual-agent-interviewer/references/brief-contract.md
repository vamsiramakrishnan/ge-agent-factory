...
`apps/factory/.gemini/skills/ge-foolproof-adk-agent-builder/references/behavior-contract.md`
...
The data specification section is consumed by `factory` CLI commands:

1. **Volume** maps to `--rows` flag on `factory generate`
2. **Entities & Schemas** map to `factory schema --add-table` JSON
...
7. **Behavior Contract** maps to `generationSpec.behaviorContract` on the slide and `mock_systems/usecase-spec.json.behaviorContract` in the workspace. `factory tools` reads it to render `app/agent.py`'s instruction, the contract tool intents in `app/tools.py`, and `evals/golden.json`.
