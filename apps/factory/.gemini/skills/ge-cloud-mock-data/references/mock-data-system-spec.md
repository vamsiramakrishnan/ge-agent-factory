...
- `factory generate`: default GE/Faker local fixture generation.
- `factory sources`: extract and classify source systems from `src/components/slides/use-cases`.
- `factory plan-data`: build `mock_data/plan/data-plan.yaml` as a collection of datastores and service adapters, then write the canonical structured Snowfakery recipe.
- `factory snowfakery-recipe`: export a Snowfakery YAML recipe under `mock_data/snowfakery/`.
- `factory data-plan`: create the currently implemented BigQuery and Cloud Storage deployable package.
...
factory data-plan --dir <workspace> --project <project> --location <location>
