...
(`apps/factory/src/workspace-validation.js`) flags each one.
...
- Agent instruction reads `"Use the fixture-backed source adapters to inspect available data. Never invent data."` — that is the no-contract stub. Re-run `factory tools --force-agent` after the contract is in place.
