...
The behavior contract is what stops the GE agent factory from emitting a hello-world / list / query shell. Every slide consumed by `ge create` (or `factory from-usecase`) must declare a `generationSpec.behaviorContract` before the factory will compile the workspace.

## Why this exists

`factory tools` derives the generated ADK agent from the behavior contract:
