...
- **CWD:** All commands run from `tmp/factory/` (or the worktree equivalent) unless absolute paths are shown.
...
```bash
cd /home/user/fde-agent-factory/tmp/factory
...
**Files:**
- New worktree at `tmp/factory/.worktrees/operator-console-phase-1`
...
```bash
cd /home/user/fde-agent-factory/tmp/factory/.worktrees/operator-console-phase-1
npm install
npm run daemon &
DAEMON_PID=$!
sleep 3
...
All remaining tasks assume CWD = `tmp/factory/.worktrees/operator-console-phase-1`.
