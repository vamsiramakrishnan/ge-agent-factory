"""Top-level synthesis orchestration + corpus promotion.
...
``synthesize_system`` resolves the NL strategy and the LLM repair hook through
the ``synthesis_sketch`` module attributes (not imported names) so tests — and
any caller — can monkeypatch ``synthesis_sketch._llm_sketch`` /
``synthesis_sketch._llm_repair`` and intercept the real call path.
"""
...
from synthesis_contract import build_contract_from_sketch
from synthesis_naming import slugify
from synthesis_seed import generate_seed
from synthesis_sketch import sketch_from_openapi, sketch_from_samples
from synthesis_validation import completeness_gaps, validate_contract
