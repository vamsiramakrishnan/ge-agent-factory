...
// ADK agents are allowed to target. Validated at generation time (see factory.mjs)
  ...
  if (!isKnownModel(value)) {
    throw new Error(
      `unknown agent model "${value}". Set GE_AGENT_MODEL to one of: ${KNOWN_AGENT_MODELS.join(", ")} ` +
        `(or add the model to apps/factory/src/known-models.js once it is enabled in the project).`,
    );
  }
  return value;
}
