...
export function buildGoogleCloudCommandGroups({
  ...
  return [
    ...
    group("datastore_plan", "Plan datastore collection", [
      ...
      "export GCS_BUCKET=\"${GOOGLE_CLOUD_PROJECT}-factory-data\"",
      `export GCS_PREFIX="factory/${sharedPrefix}/$BQ_DATASET"`,
      ...
      `factory plan-data --dir ${workspaceDir}`,
      `factory snowfakery-recipe --dir ${workspaceDir}`,
    ...
    group("olap_bigquery_gcs", "Load OLAP and blobs to BigQuery + Cloud Storage", [
      `factory data-plan --dir ${workspaceDir} --project "$GOOGLE_CLOUD_PROJECT" --location "$LOCATION" --dataset "$BQ_DATASET" --bucket "$GCS_BUCKET"`,
      `cd ${workspaceDir}`,
    ...
    group("agent_tools", "Deploy ADK agent and tools", [
      ...
      `factory deploy-status --dir ${workspaceDir} --project "$GOOGLE_CLOUD_PROJECT" --region "$LOCATION"`,
      `factory verify-live --dir ${workspaceDir} --prompt "hello"`,
    ], "ADK function tools in app/tools.py are deployed with the agent package. factory deploy is a compatibility wrapper around this agents-cli path."),
    group("registry", "Register for Gemini Enterprise", [
      `factory register --dir ${workspaceDir} --project "$GOOGLE_CLOUD_PROJECT" --region "$LOCATION" --as adk`,
    ]),
