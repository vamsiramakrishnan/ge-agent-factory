...
export async function submitFactoryRun(request) {
  ...
  const domain = String(request.department || request.domain || "hr").toLowerCase().replace(/[^a-z0-9]+/g, "-");
  ...
  if (IS_PROD) {
    ...
    const payload = {
      ...
      options: {
        ...
        displayName: title,
        description: (generationSpec && generationSpec.description) || `Specialist agent for ${useCaseId || title}.`,
        toolDescription: (generationSpec && generationSpec.description) || `Use this agent for ${domain} workflows${systems ? ` across ${systems}` : ""}.`,
        agentId
      }
    };
    ...
    const createArgs = ['src/cli.js', 'create', '--usecase', useCaseId, '--name', workspaceId, '--rows', rows, '--seed', '42', '--domain', domain];
