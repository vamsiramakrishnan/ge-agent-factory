...
const imagesBuild = defineCommand({
  ...
  args: { ...common, tag: { type: "string", description: "Explicit image tag (default: git short SHA)" }, target: { type: "positional", required: false, description: "builder = shared toolchain image (default: gateway+worker)" } },
  run: guarded(({ args }) => {
    const res = core.build(cfgFrom(args), { target: args.target, tag: args.tag, log: elog });
...
const imagesDeploy = defineCommand({
  ...
  args: { ...common, tag: { type: "string", description: "Explicit image tag (default: git short SHA)" }, target: { type: "positional", required: false, description: "gateway|worker|all (advisory; terraform apply reconciles the whole module)" } },
  run: guarded(({ args }) => {
    const res = core.deploy(cfgFrom(args), { target: args.target || "all", tag: args.tag, log: elog });
    emit(args, res, (r) => {
