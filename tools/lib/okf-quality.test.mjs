...
test("audit fixtureHash includes non-Markdown fixture data files", async()=>{
  const d=await fixture({
    "index.md":fm("Knowledge Bundle","Fixture Agent"),
    "fixtures/input.json":"{\"value\":1}\n"
  });
  const first=await auditSpec(d);
  await writeFile(join(d,"fixtures","input.json"), "{\"value\":2}\n");
  const second=await auditSpec(d);
  expect(second.hashes.fixtureHash).not.toBe(first.hashes.fixtureHash);
});
