...
| [`blueprint-library`](blueprint-library) | [README.md](blueprint-library/README.md) | private | The Agent Library engine: blueprint index generation from `okf/` bundles, search/resolve/status/related, and create-from-library workspace scaffolding. Backs `ge library`/`ge create`, the console's `/api/ge/library/*` reads, and the `factory_library_*` MCP tools. |
| [`capability-registry`](capability-registry) | [README.md](capability-registry/README.md) | private | The single capability table wiring every operator command into its surfaces — console route, CLI invocation, MCP tool, risk level, preflight requirements, observability shape — validated against the `@ge/core-api` contract at import time. Consumed by the `ge` CLI, the console (routes + client command list), the MCP server, and the docs site's command cards. |
| [`byo-systems`](byo-systems) | [README.md](byo-systems/README.md) | private | Bring-your-own-system core: known-simulator listing, synthesis-spec validation, and the `synthesize_cli.py` spawn wrapper with injected paths/interpreter. Backs `ge systems`, the console's `/api/systems/*` routes, and the `factory_systems_*` MCP tools. |
| [`connector-core`](connector-core) | [README.md](connector-core/README.md) | private | Minimal live-system connector: dial a `rest` binding read-only, with a structural write refusal (frozen `GET`/`HEAD` allowlist), auth by `env:VAR` reference resolved at call time, and the `probeRestBinding` reachability core behind `ge systems doctor --dial`. Dependency-free leaf; the full connector SDK grows from here as consumers appear. |
...
| [`core-api`](core-api) | [README.md](core-api/README.md) | private | The capability kernel: the closed vocabularies (risk levels, preflight requirement keys, observability modes, MCP param types), the client-facing `capabilityMeta()` projection, and the structural validator behind `@ge/capability-registry`. Dependency-free leaf. |
...
| [`redaction`](redaction) | [README.md](redaction/README.md) | private | The redaction module every live-system capture passes through before any byte reaches disk: default-deny credential headers, policy-driven field rules (mask/drop/hash), always-on PII detectors, and a reviewable `ge.redaction-report.v1`. Golden-fixtured — behavior changes must surface as fixture diffs. Dependency-free, pure, deterministic. |
