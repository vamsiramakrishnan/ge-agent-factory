...
import { STAGE_LOG_TYPE, RUN_COMPLETE_TYPE } from "./frames.mjs";
  ...
  if (stageName) {
    ...
    switch (ev.type) {
      ...
      case "stage_failed":
        ...
        // else terminal-failed. The ledger uses stage_failed for both; we treat
        // a present error as the blocker reason and surface a blocked state so
        // Resume is offered, falling back to failed.
        ...
        if (existing.status === "blocked") {
          ...
          signals.blockedReason = ev.error || signals.blockedReason;
        }
        break;
      default:
