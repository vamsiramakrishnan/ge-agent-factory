...
describe("reduceFrames", () => {
  ...
  test("stage_error (worker real-throw frame) renders via the failed path", () => {
    const acc = createRunAccumulator();
    applyEvent({ type: "stage_started", stage: "deploy" }, acc.stages, acc.signals);
    // message in top-level `error`, full stack/cmd in `data` (cloud normalizer keeps data).
    applyEvent(
      { type: "stage_error", stage: "deploy", status: "failed", error: "AccessDenied: bucket", data: { message: "AccessDenied: bucket", stack: "at load.sh" } },
      acc.stages,
      acc.signals,
    );
    expect(acc.signals.status).toBe("failed");
    expect(acc.signals.blockedReason).toBe("AccessDenied: bucket");
    expect(orderStages(acc.stages, acc.firstSeen)[0].status).toBe("failed");
  });

  test("stage_error falls back to data.message when top-level error is absent", () => {
    const acc = createRunAccumulator();
    applyEvent({ type: "stage_error", stage: "deploy", status: "failed", data: { message: "boom from data" } }, acc.stages, acc.signals);
    expect(acc.signals.status).toBe("failed");
    expect(acc.signals.blockedReason).toBe("boom from data");
  });
...
describe("store", () => {
  test("factoryEventToLedgerOp maps the generator stream", () => {
    ...
    expect(factoryEventToLedgerOp({ type: "stage_error", stage: "build", error: "boom" })).toMatchObject({ kind: "transition", stage: "build", status: "failed", error: "boom" });
    expect(factoryEventToLedgerOp({ type: "stage_failed", stage: "build", data: { message: "nested" } })).toMatchObject({ kind: "transition", stage: "build", status: "failed", error: "nested" });
