...
export const CONTROL_CLASS =
  "w-full rounded-md border border-outline/20 bg-surface-container px-3 py-2 text-sm text-on-surface outline-none focus:border-primary/50";
...
export function Field({ label, hint, className, children }: FieldProps) {
  return (
    <label className={cx("block", className)}>
      ...
      {children}
      {hint && <span className="mt-1 block text-[11px] text-secondary/80">{hint}</span>}
    </label>
  );
}
